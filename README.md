...
- backend на .NET 9: `1605/1605` unit tests;
- API Release build: без ошибок и предупреждений;
- frontend unit tests: `197/197`;
- frontend typecheck и production build: OK;
- frontend dependency audit: `0 vulnerabilities`; React 19.2.8 и React Router 8.3.0 проверены на Node.js 22.22.0;
- Playwright E2E: public, cabinet, admin, all-screens, mobile и console smoke проходят `282/282`; responsive matrix проверяет 25 конфигураций `305x568..2560x1440`, включая mobile landscape и точные пары `N/N+1` для всех CSS-breakpoints;
...
- кабинет и админка явно форматируют пользовательские даты по `ru-RU`; история заказов, платежей и обращений не показывает технические enum-значения вместо русских подписей;
...
- admin production bundle: `5` JS chunks, largest `277529`, total raw `586266/586752`, gzip `156003/156672` bytes; build budget fail-closed проверяет largest/total raw/gzip;
...
- управляемые редакторы FAQ, контента и сценариев отклоняют no-op, сохраняют более новый локальный черновик при delayed `409`, а user PATCH принимает эквивалентный SQLite concurrency timestamp с другим offset;
- тарифы, реферальные программы и релизы приложения отклоняют no-op без revision/audit churn; UI блокирует пустую отправку, а delayed tariff conflict сохраняет более новый локальный черновик;
- 3x-ui panel/inbound editors отклоняют no-op до audit/revision и remote provider mutation; delayed conflict сохраняет более новый локальный черновик;
- платежные аккаунты используют persisted revision, отклоняют stale/no-op update и enabled-state без ложного audit, а delayed conflict сохраняет более новый локальный черновик;
- более новые черновики payment provider, tariff, referral, release, VPN panel и inbound после delayed `409` получают актуальную revision и повторно сохраняются без цикла конфликтов;
- повторные и недопустимые server mode-actions отклоняются без mutation, stale revision возвращает `409` до semantic validation, а UI показывает только допустимые переходы текущего состояния;
...
- archived VPN server сохраняет status и последнюю диагностику, но direct health отклоняется без local churn, а mutation toolbar отсутствует;
...
- changelog, финальный runbook, release decision, roadmap, продуктовый UI-roadmap и журнал ошибок синхронизированы с разделом "Что нового": `2026-08-24-deploy-production-smtp-preflight`, версия `0.751.0`;
- production deploy fail-fast проверяет обязательные SMTP-настройки до upload и остановки systemd API; реальные SMTP-значения остаются внешней настройкой GitHub secret;
