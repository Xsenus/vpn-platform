...
- backend на .NET 9: `1146/1146` unit tests;
...
- changelog, финальный runbook, release decision, roadmap, продуктовый UI-roadmap и журнал ошибок синхронизированы с разделом "Что нового": `2026-08-12-access-sync-read-failure`, версия `0.640.0`;
- lifecycle различает read-only provider sync failure, local persistence failure и необратимый reset: status/timestamp сохраняются для чтения, staged success удаляется при commit failure, а reset uncertainty получает `SyncRequired`;
