...
- backend на .NET 9: `1196/1196` unit tests;
...
- changelog, финальный runbook, release decision, roadmap, продуктовый UI-roadmap и журнал ошибок синхронизированы с разделом "Что нового": `2026-08-12-provisioning-credential-preflight`, версия `0.646.0`;
- legacy SSH key path принимает только абсолютный Unix path без control/quote-символов, а provisioning queue до создания run отклоняет orphan references, placeholders, password credentials и иные состояния, которые runner не может материализовать;
