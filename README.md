...
- backend на .NET 9: `1144/1144` unit tests;
- API Release build: без ошибок и предупреждений;
- frontend unit tests: `136/136`;
- frontend typecheck и production build: OK;
- frontend dependency audit: `0 vulnerabilities`; React 19.2.8 и React Router 8.3.0 проверены на Node.js 22.22.0;
- Playwright E2E: public, cabinet, admin, all-screens, mobile и console smoke проходят `218/218`; responsive matrix проверяет 25 конфигураций `305x568..2560x1440`, включая mobile landscape и точные пары `N/N+1` для всех CSS-breakpoints;
...
- changelog, финальный runbook, release decision, roadmap, продуктовый UI-roadmap и журнал ошибок синхронизированы с разделом "Что нового": `2026-08-12-access-sync-reset-persistence`, версия `0.639.0`;
- lifecycle enable/disable компенсирует успешную provider-мутацию, а sync/reset после local persistence failure удаляет staged success evidence, сохраняет исходный sync-state или ставит `SyncRequired` для необратимого reset;
- roadmap progress: `652/672` closed, readiness `97.0%`, `20` remaining, `19` open, `1` in progress and `0` blocked;
