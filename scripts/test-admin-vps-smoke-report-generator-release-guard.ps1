...
$generatorPath = Resolve-RepoPath "scripts/new-admin-vps-smoke-report.ps1"
$tmpDirectory = Resolve-RepoPath "tmp"
New-Item -ItemType Directory -Force -Path $tmpDirectory | Out-Null

$reportPath = Join-Path $tmpDirectory "admin-vps-smoke-generator-unknown-release-id.json"
...
try {
    ...
    $output = & powershell -NoProfile -ExecutionPolicy Bypass -File $generatorPath `
        ...
        -Operator "admin-vps-smoke-generator-release-guard" `
    ...
    Write-Output "admin vps smoke generator release guard valid"
}
finally {
    if (Test-Path -LiteralPath $reportPath) {
        Remove-Item -LiteralPath $reportPath -Force
