...
function New-RootedEntryArchive {
    ...
    try {
        ...
        Add-ZipTextEntry -Archive $archive -EntryName "C:\SHA256SUMS.txt" -Content "rooted placeholder"
    }
    finally {
        if ($archive -ne $null) {
            $archive.Dispose()
...
$archivePath = Join-Path $tmpDirectory "production-evidence-handoff-package-archive-rooted-entry-guard.zip"
...
try {
    ...
    New-RootedEntryArchive -ArchivePath $archivePath

    [void](Assert-FailsWith -ExpectedMessage "unexpected entry" -Action {
            & $validatorPath -ArchivePath $archivePath -WriteJson
        })

    Write-Output "production evidence handoff package archive rooted entry guard valid"
}
finally {
    if (Test-Path -LiteralPath $archivePath) {
        Remove-Item -LiteralPath $archivePath -Force
