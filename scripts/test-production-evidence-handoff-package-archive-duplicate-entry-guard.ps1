...
function New-DuplicateEntryArchive {
    ...
    try {
        ...
        Add-ZipTextEntry -Archive $archive -EntryName "SHA256SUMS.txt" -Content "duplicated placeholder"
    }
    finally {
        if ($archive -ne $null) {
            $archive.Dispose()
...
$archivePath = Join-Path $tmpDirectory "production-evidence-handoff-package-archive-duplicate-entry-guard.zip"
...
try {
    ...
    New-DuplicateEntryArchive -ArchivePath $archivePath

    [void](Assert-FailsWith -ExpectedMessage "duplicated entry" -Action {
            & $validatorPath -ArchivePath $archivePath -WriteJson
        })

    Write-Output "production evidence handoff package archive duplicate entry guard valid"
}
finally {
    if (Test-Path -LiteralPath $archivePath) {
        Remove-Item -LiteralPath $archivePath -Force
