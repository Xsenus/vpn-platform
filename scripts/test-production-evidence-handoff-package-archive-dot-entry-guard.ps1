...
function New-DotEntryArchive {
    ...
    try {
        ...
        Add-ZipTextEntry -Archive $archive -EntryName "." -Content "dot placeholder"
    }
    finally {
        if ($archive -ne $null) {
            $archive.Dispose()
...
$archivePath = Join-Path $tmpDirectory "production-evidence-handoff-package-archive-dot-entry-guard.zip"
...
try {
    ...
    New-DotEntryArchive -ArchivePath $archivePath

    [void](Assert-FailsWith -ExpectedMessage "file name only" -Action {
            & $validatorPath -ArchivePath $archivePath -WriteJson
        })

    Write-Output "production evidence handoff package archive dot entry guard valid"
}
finally {
    if (Test-Path -LiteralPath $archivePath) {
        Remove-Item -LiteralPath $archivePath -Force
