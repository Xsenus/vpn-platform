...
function New-WhitespaceEntryArchive {
    ...
    try {
        ...
        Add-ZipTextEntry -Archive $archive -EntryName " " -Content "whitespace placeholder"
    }
    finally {
        if ($archive -ne $null) {
            $archive.Dispose()
...
$archivePath = Join-Path $tmpDirectory "production-evidence-handoff-package-archive-whitespace-entry-guard.zip"
...
try {
    ...
    New-WhitespaceEntryArchive -ArchivePath $archivePath

    [void](Assert-FailsWith -ExpectedMessage "unexpected entry" -Action {
            & $validatorPath -ArchivePath $archivePath -WriteJson
        })

    Write-Output "production evidence handoff package archive whitespace entry guard valid"
}
finally {
    if (Test-Path -LiteralPath $archivePath) {
        Remove-Item -LiteralPath $archivePath -Force
