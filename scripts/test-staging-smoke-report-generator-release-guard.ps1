...
$generatorPath = Resolve-RepoPath "scripts/new-staging-smoke-report.ps1"
$tmpDirectory = Resolve-RepoPath "tmp"
New-Item -ItemType Directory -Force -Path $tmpDirectory | Out-Null

$reportPath = Join-Path $tmpDirectory "staging-smoke-generator-unknown-release-id.json"
...
try {
    ...
    $output = & powershell -NoProfile -ExecutionPolicy Bypass -File $generatorPath `
        ...
        -Operator "staging-smoke-generator-release-guard" `
    ...
    Write-Output "staging smoke generator release guard valid"
}
finally {
    if (Test-Path -LiteralPath $reportPath) {
        Remove-Item -LiteralPath $reportPath -Force
