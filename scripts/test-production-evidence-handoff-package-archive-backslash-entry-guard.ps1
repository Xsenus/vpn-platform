...
function New-BackslashEntryArchive {
    ...
    try {
        ...
        Add-ZipTextEntry -Archive $archive -EntryName "nested\SHA256SUMS.txt" -Content "backslash placeholder"
    }
    finally {
        if ($archive -ne $null) {
            $archive.Dispose()
...
$archivePath = Join-Path $tmpDirectory "production-evidence-handoff-package-archive-backslash-entry-guard.zip"
...
try {
    ...
    New-BackslashEntryArchive -ArchivePath $archivePath

    [void](Assert-FailsWith -ExpectedMessage "unexpected entry" -Action {
            & $validatorPath -ArchivePath $archivePath -WriteJson
        })

    Write-Output "production evidence handoff package archive backslash entry guard valid"
}
finally {
    if (Test-Path -LiteralPath $archivePath) {
        Remove-Item -LiteralPath $archivePath -Force
