...
function New-CaseMismatchArchive {
...
$archivePath = Join-Path $tmpDirectory "production-evidence-handoff-package-archive-entry-case-guard.zip"
...
try {
    ...
    New-CaseMismatchArchive -ArchivePath $archivePath

    [void](Assert-FailsWith -ExpectedMessage "unexpected entry" -Action {
            & $validatorPath -ArchivePath $archivePath -WriteJson
        })

    Write-Output "production evidence handoff package archive entry case guard valid"
}
finally {
    if (Test-Path -LiteralPath $archivePath) {
        Remove-Item -LiteralPath $archivePath -Force
