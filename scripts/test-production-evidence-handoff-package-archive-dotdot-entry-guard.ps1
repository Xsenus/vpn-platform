...
function New-DotDotEntryArchive {
    ...
    try {
        ...
        Add-ZipTextEntry -Archive $archive -EntryName ".." -Content "dotdot placeholder"
    }
    finally {
        if ($archive -ne $null) {
            $archive.Dispose()
...
$archivePath = Join-Path $tmpDirectory "production-evidence-handoff-package-archive-dotdot-entry-guard.zip"
...
try {
    ...
    New-DotDotEntryArchive -ArchivePath $archivePath

    [void](Assert-FailsWith -ExpectedMessage "file name only" -Action {
            & $validatorPath -ArchivePath $archivePath -WriteJson
        })

    Write-Output "production evidence handoff package archive dotdot entry guard valid"
}
finally {
    if (Test-Path -LiteralPath $archivePath) {
        Remove-Item -LiteralPath $archivePath -Force
