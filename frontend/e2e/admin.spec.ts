...
test('support agent keeps channel-aware conversation lifecycle across reload', async ({ page }) => {
  ...
  await expect(supportPanel.getByText('Администратор · внутренняя заметка', { exact: true })).toBeVisible()
...
test('admin serializes VPN infrastructure commands across parent resources', async ({ page }) => {
  ...
  await expect(page.getByText(/Синхронизация Успешно/)).toBeVisible()
...
test('admin serializes finance commands across provider and payment resources', async ({ page }) => {
  ...
  await expect(page.getByText('Заказ order-e2: последний платеж payment- проверен, статус Успешно.')).toBeVisible()
...
test('admin payment recheck and refunds persist across reload', async ({ page }) => {
  ...
  await paymentsPanel.getByRole('button', { name: 'Подтвердить' }).click()
  await expect(page.getByText('Возврат rf-e2e-1: Успешно')).toBeVisible()
  ...
  await paymentsPanel.getByRole('button', { name: 'Подтвердить' }).click()
  await expect(page.getByText('Возврат rf-e2e-2: Успешно')).toBeVisible()
...
test('admin reconciles a pending refund and applies its amount once', async ({ page }) => {
  ...
  await expect(page.getByText('Возврат rf-pending-e2e проверен: Успешно')).toBeVisible()
...
test('admin recovers an uncertain refund create without manual reload', async ({ page }) => {
  ...
  await expect(page.getByText('Возврат pending:refund-uncertain-e2e: Неизвестно')).toBeVisible()
  ...
  await paymentsPanel.getByRole('button', { name: 'Подтвердить' }).click()
  await expect(page.getByText('Возврат rf-e2e-1 повторён: Успешно')).toBeVisible()
  await expect(page.locator('#payment-payment-e2e')).toContainText('возвращено 200 RUB')
  await expect(paymentsPanel.getByText('Возврат 200 RUB · rf-e2e-1')).toBeVisible()
...
test('admin VPN infrastructure supports secure managed lifecycle', async ({ page }) => {
  ...
  await expect(serverRow.getByRole('button', { name: 'Закрыть набор' })).toBeVisible()
...
test('admin panel covers login and critical operational mutations across all sections', async ({ page }, testInfo) => {
  ...
  await orderRow.getByRole('button', { name: 'Проверить оплату' }).click()
  await expect(page.getByText(/последний платеж .* проверен, статус Успешно/)).toBeVisible()
  ...
  await expect(page.getByText(/Платеж .* проверен: Успешно/)).toBeVisible()
  ...
  await expect(page.getByText('Проверка панели: Работает (2.4.9)')).toBeVisible()
  await expect(euPanelRow).toContainText('Емкость 11/1000')
  await expect(usPanelRow).toContainText('Емкость 5/1000')
  await usPanelRow.getByRole('button', { name: 'Синхронизировать' }).click()
  await expect(page.getByText('Синхронизация Успешно: {"clients":1}')).toBeVisible()
