...
function validateVpnPanelForm(form: CreateVpnPanelPayload, isEditing: boolean) {
  ...
  if (form.authenticationMode === 'PasswordSession' && !form.login.trim()) errors.push('Укажите логин 3x-ui панели.')
...
export function App() {
  ...
  const editVpnPanel = (panel: VpnPanelDto) => {
    ...
    setVpnPanelForm({
      ...
      authenticationMode: panel.authenticationMode || 'PasswordSession',
  ...
  const handleDeleteVpnPanel = (panel: VpnPanelDto) => runAction('panels', `panel-delete-${panel.id}`, async (action) => {
    ...
    setNotice(result.archived
      ? `Панель ${panel.name} архивирована и сохранена в истории: связей ${result.linkedInbounds + result.linkedClients + result.linkedSyncRuns + result.linkedHealthChecks}.`
          ...
          <form hidden={!canWriteSection('panels')} aria-busy={vpnPanelFormActionBusy} onSubmit={(event) => { event.preventDefault(); void handleSaveVpnPanel() }}>
            <fieldset className="form-section">
              ...
              <div className="form-grid">
                ...
                <label><span>Аутентификация</span><select value={vpnPanelForm.authenticationMode} onChange={(e) => updateVpnPanelForm('authenticationMode', e.target.value)}><option value="ApiToken">API token</option><option value="PasswordSession">Логин и пароль</option></select></label>
                <label><span>Логин</span><input value={vpnPanelForm.login} onChange={(e) => updateVpnPanelForm('login', e.target.value)} placeholder="admin" maxLength={200} disabled={vpnPanelForm.authenticationMode === 'ApiToken'} /></label>
                <PasswordField label={vpnPanelForm.authenticationMode === 'ApiToken' ? 'API token' : 'Пароль панели'} value={vpnPanelForm.password ?? ''} maxLength={4096} onChange={(value) => updateVpnPanelForm('password', value)} placeholder={editingVpnPanelId ? 'Оставьте пустым, чтобы сохранить текущий секрет' : 'Хранится зашифрованным'} autoComplete="new-password" />
              </div>
            </fieldset>
            <fieldset className="form-section">
