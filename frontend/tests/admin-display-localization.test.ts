...
test('admin display formatter localizes bounded operational metadata', () => {
  ...
  assert.equal(formatAdminDisplayLabel('least-loaded'), 'Наименее загруженный сервер')
  assert.equal(formatAdminDisplayLabel('default'), 'Основное inbound-правило')
  assert.equal(formatAdminDisplayLabel('create_subscription_and_access'), 'Создать подписку и VPN-доступ')
  assert.equal(formatAdminDisplayLabel('keep_order_pending'), 'Оставить заказ в ожидании')
...
test('admin user-facing metadata does not interpolate technical enum values directly', () => {
  ...
  assert.doesNotMatch(source, /сервер \{scenario\.serverSelectionRule\}/)
  assert.doesNotMatch(source, /Оплата: \{scenario\.onPaymentSucceeded\}/)
