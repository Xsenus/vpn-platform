...
import { cabinetSessionEndedMessage, isCabinetAccessTokenExpired, isCabinetSessionRejected } from '../apps/cabinet/src/cabinet-session.ts'
...
test('cabinet session rejects unauthorized and forbidden API responses only', () => {
  ...
  assert.equal(isCabinetAccessTokenExpired(new ApiClientError('unauthorized', 401, null)), true)
  assert.equal(isCabinetAccessTokenExpired(new ApiClientError('forbidden', 403, null)), false)
  assert.equal(isCabinetAccessTokenExpired(new Error('network')), false)
