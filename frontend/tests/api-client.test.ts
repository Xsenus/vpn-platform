...
test('frontend sources keep sandbox E2E surfaces safe and user-friendly', () => {
  ...
  assert.match(cabinetSource, /payment\.confirmationUrl && canOpenPaymentConfirmation\(payment\.status\)/)
  assert.match(cabinetSource, /retryPaymentState\.order\.status === 'PendingPayment'/)
