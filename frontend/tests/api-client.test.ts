...
test('frontend sources keep sandbox E2E surfaces safe and user-friendly', () => {
  ...
  const adminSource = readFileSync(new URL('../apps/admin-panel/src/App.tsx', import.meta.url), 'utf8')
  const adminServerActionsSource = readFileSync(new URL('../apps/admin-panel/src/admin-server-actions.ts', import.meta.url), 'utf8')
  ...
  assert.match(adminSource, /getServerStateActionAvailability/)
  assert.match(adminSource, /stateActions\.canEnterMaintenance/)
  assert.match(adminSource, /stateActions\.canLeaveMaintenance/)
  assert.match(adminSource, /stateActions\.canDisableAllocation/)
  assert.match(adminSource, /stateActions\.canEnableAllocation/)
  assert.match(adminSource, /stateActions\.canDisable/)
  assert.match(adminSource, /stateActions\.canDelete/)
  assert.match(adminServerActionsSource, /status !== 'Disabled' && status !== 'Archived'/)
  assert.match(adminSource, /server\.usedCapacity > 0/)
  assert.match(adminServerActionsSource, /canDelete: status !== 'Archived' && usedCapacity === 0/)
