...
test('frontend sources include app version gate and admin release editor', () => {
  ...
  assert.match(cabinetAppSource, /const cabinetDataReady = !token \|\| profile !== null/)
...
test('Stage 9 UI source includes MVP polish surfaces and safety affordances', () => {
  ...
  assert.match(publicSource, /Заказ создан, оплата не подготовлена/)
  ...
  assert.match(publicSource, /errorContentKey\s*\?\s*content\(errorContentKey\)\s*:\s*error/)
  assert.match(publicSource, /setErrorContentKey\('home\.errors\.tariffsLoad'\)/)
  assert.match(publicSource, /setErrorContentKey\('home\.errors\.paymentProvidersLoad'\)/)
