...
public class ProductionReadinessGateTests
{
    ...
    [Fact]
    public void Production_Readiness_Assertion_Ci_Step_Summary_Should_Cleanup_Default_Output()
    {
        var root = FindRepositoryRoot();
        var script = File.ReadAllText(Path.Combine(root, "scripts", "test-production-readiness-assertion-ci-step-summary.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production readiness assertion CI step summary removes its autogenerated default output directory and empty `tmp` after ordinary local runs", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-128`", roadmap, StringComparison.Ordinal);
    }
        ...
        foreach (var expected in new[]
                 {
                     ...
                     "summaryPath",
                     "jsonSummaryPath",
                     "must match validated summary path",
    ...
    [Fact]
    public void Production_Evidence_Bundle_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-bundle-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence bundle latest release guard removes its autogenerated `tmp/production-evidence-bundle-stale-release-guard` directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-115`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Bundle_Generator_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-bundle-generator-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence bundle generator release guard removes its autogenerated empty `tmp` directory after a local run", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-125`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Manifest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-manifest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence manifest release guard removes its autogenerated `tmp/production-evidence-manifest-unknown-release-id` directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-116`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Archive_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-archive-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence archive release guard removes its autogenerated `tmp/production-evidence-archive-unknown-release-id` directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-117`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Receipt_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-receipt-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff receipt release guard removes its autogenerated `tmp/production-evidence-handoff-receipt-unknown-release-id` directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-118`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Receipt_Verified_Files_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-receipt-verified-files-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff receipt verified files guard removes its autogenerated `tmp/production-evidence-handoff-receipt-verified-files-guard` bundle directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-121`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Receipt_Markdown_Verified_Files_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-receipt-markdown-verified-files-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff receipt Markdown verified files guard removes its autogenerated `tmp/production-evidence-handoff-receipt-markdown-verified-files-guard` bundle directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-122`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Checklist_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-checklist-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff checklist release guard removes its autogenerated `tmp/production-evidence-handoff-checklist-unknown-release-id` directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-119`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Checklist_Markdown_Gates_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-checklist-markdown-gates-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff checklist Markdown gates guard removes its autogenerated `tmp/production-evidence-handoff-checklist-markdown-gates-guard` bundle directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-123`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Package_Markdown_Files_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-package-markdown-files-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff package Markdown files guard removes its autogenerated `tmp/production-evidence-handoff-package-markdown-files-guard` bundle directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-124`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Package_Archive_Long_Path_Should_Cleanup_Default_Output()
    {
        var root = FindRepositoryRoot();
        var script = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-package-archive-long-path.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Default long-path regression run removes its autogenerated `tmp/production-evidence-handoff-package-archive-long-release-id-path-regression-test` output", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-106`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Readiness_Assertion_Ci_Regression_Should_Cleanup_Default_Output()
    {
        var root = FindRepositoryRoot();
        var script = File.ReadAllText(Path.Combine(root, "scripts", "test-production-readiness-assertion-ci-regression.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        var roadmap = File.ReadAllText(Path.Combine(root, "docs", "PRODUCT_COMPLETION_ROADMAP.md"));
        ...
        Assert.Contains("Default production readiness assertion CI regression run removes its autogenerated `tmp/production-readiness-assertion-ci-regression-test` output", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-107`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Checklist_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-checklist-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff checklist latest release guard removes its autogenerated `tmp/production-evidence-handoff-checklist-stale-release-guard.json` and `.md` output", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-110`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Package_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-package-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff package latest release guard removes its autogenerated `tmp/production-evidence-handoff-package-stale-release-guard` package directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-120`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Package_Archive_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-package-archive-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff package archive latest release guard removes its autogenerated `tmp/production-evidence-handoff-package-archive-stale-release-guard.zip`, package directory and empty `tmp` directory", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-114`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Package_Archive_Flow_Result_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-package-archive-flow-result-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff package archive flow result latest release guard removes its autogenerated `tmp/production-evidence-handoff-package-archive-flow-result-stale-release-guard.json` output", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-111`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Package_Archive_Ci_Regression_Result_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-package-archive-ci-regression-result-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff package archive CI regression result latest release guard removes its autogenerated `tmp/production-evidence-handoff-package-archive-ci-regression-result-stale-release-guard.json` output", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-112`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Evidence_Handoff_Package_Archive_Ci_Summary_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-evidence-handoff-package-archive-ci-summary-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production evidence handoff package archive CI summary latest release guard removes its autogenerated `tmp/production-evidence-handoff-package-archive-ci-summary-stale-release-guard.json` and `.md` output", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-113`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Readiness_Summary_Should_Self_Link_To_Validated_Files()
    {
        var root = FindRepositoryRoot();
        var generator = File.ReadAllText(Path.Combine(root, "scripts", "new-production-readiness-summary.ps1"));
        var validator = File.ReadAllText(Path.Combine(root, "scripts", "validate-production-readiness-summary.ps1"));
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-readiness-summary-self-link-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        var roadmap = File.ReadAllText(Path.Combine(root, "docs", "PRODUCT_COMPLETION_ROADMAP.md"));
        ...
        {
            Assert.Contains(expected, generator, StringComparison.OrdinalIgnoreCase);
        }

        foreach (var expected in new[]
                 {
                     "Resolve-SummaryLinkedPath",
                     "Assert-SamePath",
                     "must match validated summary path"
                 })
        {
        ...
        foreach (var expected in new[]
                 {
                     "tmp/other-production-readiness-summary.md",
                     "Validator accepted mismatched summaryPath",
                     "production readiness summary self-link guard valid"
                 })
        {
            Assert.Contains(expected, regression, StringComparison.OrdinalIgnoreCase);
        }

        Assert.Contains("test-production-readiness-summary-self-link-guard.ps1", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-131`", roadmap, StringComparison.Ordinal);
    }

    [Fact]
    public void Production_Readiness_Summary_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-readiness-summary-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production readiness summary latest release guard removes its autogenerated `tmp/production-readiness-summary-stale-release-guard.md` and `.json` output", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-109`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Production_Readiness_Assertion_Result_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-production-readiness-assertion-result-latest-release-guard.ps1"));
        var docs = File.ReadAllText(Path.Combine(root, "docs", "production-readiness-gate.md"));
        ...
        Assert.Contains("Production readiness assertion result latest release guard removes its autogenerated `tmp/production-readiness-assertion-result-stale-release-guard.json` output", docs, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-108`", roadmap, StringComparison.Ordinal);
    }
