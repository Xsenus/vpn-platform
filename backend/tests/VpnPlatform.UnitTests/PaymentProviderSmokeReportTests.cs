...
public class PaymentProviderSmokeReportTests
{
    ...
    [Fact]
    public void Smoke_Report_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-payment-provider-smoke-report-latest-release-guard.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "payment-provider-smoke.md"));
        ...
        Assert.Contains("Payment provider smoke latest release guard removes its autogenerated `tmp/payment-provider-smoke-stale-release-guard.json` report and empty `tmp` directory after a local run", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P0-PAY-018`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Smoke_Report_Generator_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-payment-provider-smoke-report-generator-release-guard.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "payment-provider-smoke.md"));
        ...
        Assert.Contains("Payment provider smoke generator release guard removes its autogenerated empty `tmp` directory after a local run", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P0-PAY-017`", roadmap, StringComparison.Ordinal);
    }
