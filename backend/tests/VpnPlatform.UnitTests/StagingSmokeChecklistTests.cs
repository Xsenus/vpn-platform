...
public class StagingSmokeChecklistTests
{
    ...
    public void Staging_Smoke_Report_Generator_Should_Create_Safe_Blocked_Report()
    {
        ...
        Assert.Contains("test-staging-smoke-report-generator-release-guard.ps1", guide, StringComparison.OrdinalIgnoreCase);
    ...
    [Fact]
    public void Staging_Smoke_Report_Generator_Should_Reject_Unknown_Manual_ReleaseId()
    {
        var root = FindRepositoryRoot();
        var script = File.ReadAllText(Path.Combine(root, "scripts", "new-staging-smoke-report.ps1"));
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-staging-smoke-report-generator-release-guard.ps1"));
        ...
        Assert.Contains("staging smoke generator release guard valid", regression, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P9-TST-007G`", roadmap, StringComparison.Ordinal);
    }

    [Fact]
    public void Staging_Smoke_Report_Generator_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-staging-smoke-report-generator-release-guard.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "staging-smoke-checklist.md"));
        ...
        Assert.Contains("Staging smoke generator release guard removes its autogenerated empty `tmp` directory after a failed unknown-release local run", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P9-TST-007I`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Staging_Smoke_Report_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-staging-smoke-report-latest-release-guard.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "staging-smoke-checklist.md"));
        ...
        Assert.Contains("Staging smoke latest release guard removes its autogenerated `tmp/staging-smoke-stale-release-guard.json` report and empty `tmp` directory", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P9-TST-007H`", roadmap, StringComparison.Ordinal);
    }
