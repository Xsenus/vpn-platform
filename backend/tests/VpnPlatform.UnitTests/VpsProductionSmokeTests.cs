...
public class VpsProductionSmokeTests
{
    ...
    [Fact]
    public void Vps_Production_Smoke_Report_Generator_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-vps-production-smoke-report-generator-release-guard.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "vps-production-smoke.md"));
        ...
        Assert.Contains("VPS production smoke generator release guard removes its autogenerated empty `tmp` directory after a local run", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-126`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Vps_Production_Smoke_Report_Latest_Release_Guard_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-vps-production-smoke-report-latest-release-guard.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "vps-production-smoke.md"));
        ...
        Assert.Contains("VPS production smoke latest release guard removes its autogenerated `tmp/vps-production-smoke-stale-release-guard.json` report and empty `tmp` directory after a local run", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P11-ACC-127`", roadmap, StringComparison.Ordinal);
    }
