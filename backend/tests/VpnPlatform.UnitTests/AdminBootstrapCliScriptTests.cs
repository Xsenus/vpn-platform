...
public class AdminBootstrapCliScriptTests
{
    ...
    [Fact]
    public void Admin_Bootstrap_Wrapper_Regression_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-admin-bootstrap-wrapper.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "admin-bootstrap.md"));
        ...
        Assert.Contains("test-admin-bootstrap-wrapper.ps1", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P0-ADMIN-001CA`", roadmap, StringComparison.Ordinal);
    }
    ...
    [Fact]
    public void Local_Admin_Vps_Bootstrap_Smoke_Wrapper_Regression_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-local-admin-vps-bootstrap-smoke-wrapper.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "admin-bootstrap.md"));
        var roadmap = File.ReadAllText(Path.Combine(root, "docs", "PRODUCT_COMPLETION_ROADMAP.md"));
        ...
        Assert.Contains("test-local-admin-vps-bootstrap-smoke-wrapper.ps1", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("default output directory", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P0-ADMIN-001CD`", roadmap, StringComparison.Ordinal);
    }
        ...
        foreach (var expected in new[]
                 {
                     ...
                     "password-env-name-safe",
                     "passwordEnvName must be a safe environment variable name containing PASSWORD",
                     "Get-ReportPathValue",
                     ...
                     "provider must be Sqlite when localSqlite is true",
                     "admin-vps-bootstrap-smoke-readiness-report.json",
        ...
        foreach (var expected in new[]
                 {
                     ...
                     "mismatched-readiness-password-env-name-safe",
                     "missing-password",
                     "missing-confirm-bootstrap-reset",
                     "missing-connection-string",
                     "leaked password",
        ...
        foreach (var expected in new[]
                 {
                     ...
                     "Smoke artifact should not exist",
                      ...
                      "Readiness report operator should be",
    ...
    [Fact]
    public void Admin_Vps_Bootstrap_Release_Guard_Harnesses_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        ...
        foreach (var scriptName in new[]
                 {
                     "test-admin-vps-bootstrap-smoke-latest-release-guard.ps1",
                     "test-admin-vps-bootstrap-smoke-evidence-latest-release-guard.ps1",
                     "test-admin-vps-bootstrap-readiness-release-guard.ps1"
                 })
        {
        ...
        Assert.Contains("Admin VPS bootstrap release guard harnesses remove their autogenerated reports and empty `tmp` directory after local runs", guide, StringComparison.OrdinalIgnoreCase);

        foreach (var roadmapId in new[] { "P0-ADMIN-001BX", "P0-ADMIN-001BY", "P0-ADMIN-001BZ" })
        {
    ...
    [Fact]
    public void Admin_Vps_Bootstrap_Smoke_Readiness_Regression_Should_Cleanup_Default_Tmp()
    {
        var root = FindRepositoryRoot();
        var regression = File.ReadAllText(Path.Combine(root, "scripts", "test-admin-vps-bootstrap-smoke-readiness.ps1"));
        var guide = File.ReadAllText(Path.Combine(root, "docs", "admin-bootstrap.md"))
                    + File.ReadAllText(Path.Combine(root, "docs", "admin-vps-smoke.md"));
        var roadmap = File.ReadAllText(Path.Combine(root, "docs", "PRODUCT_COMPLETION_ROADMAP.md"));
        ...
        Assert.Contains("test-admin-vps-bootstrap-smoke-readiness.ps1", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("readiness regression removes its autogenerated reports and empty `tmp` directory", guide, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("[x] `P0-ADMIN-001CF`", roadmap, StringComparison.Ordinal);
    }
