...
- Browser console smoke: `180/180`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-admin-finance-resource-owner`, версия `0.622.0`; payment-provider edit/toggle/check и order/direct payment recheck/refund используют общие identity-safe resource owners, поэтому sibling finance-команды не пересекают provider state mutations. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
