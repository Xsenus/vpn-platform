...
- Browser console smoke: `208/208`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-12-public-payment-link-expiry`, версия `0.633.0`; public account автоматически скрывает payment redirect по checkout-order expiry без ручного refresh и показывает новый заказ. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
