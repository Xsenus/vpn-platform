...
- Latest "Что нового": `2026-06-22-admin-vps-bootstrap-readiness-metadata-link`, версия `0.216.0`; `scripts/validate-admin-vps-bootstrap-smoke-report.ps1 -RequirePassed` валидирует связанный readiness report, сверяет readiness/preflight/browser smoke evidence с итоговым bootstrap smoke report и fail-closed ловит отсутствующий readiness link, mismatch provider, password env, `localSqlite` и `confirmBootstrapReset`; regression harness покрывает `mismatched-readiness-provider`, `mismatched-readiness-password-env-name`, `mismatched-readiness-local-sqlite`, `mismatched-readiness-confirm-bootstrap-reset`, `missing-bootstrap-readiness-report-link`, `mismatched-readiness-bootstrap-report-path` и прежние path/admin/release checks; production readiness gate и live VPS/staging evidence все еще требуются.
