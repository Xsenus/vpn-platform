...
- Browser console smoke: `178/178`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-admin-vpn-infrastructure-resource-owner`, версия `0.621.0`; panel/inbound/client и server/provisioning команды используют иерархические identity-safe resource owners, поэтому sibling commands не пересекают remote state mutations, а client migration резервирует source/target ресурсы. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
