...
- Browser console smoke: `174/174`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-admin-support-mutation-resource-owner`, версия `0.619.0`; status/reply/internal note одного admin support conversation используют общий identity-safe resource owner, поэтому sibling mutation не пересекает optimistic revision и сохраняет свой draft. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
