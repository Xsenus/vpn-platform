...
- Latest "Что нового": `2026-06-18-production-readiness-summary-validator`, версия `0.128.0`; production readiness gate агрегирует полный пакет evidence reports, `new-production-evidence-bundle.ps1` создает четыре безопасных черновика одной командой, `new-production-readiness-summary.ps1` собирает Markdown/JSON summary, а `validate-production-readiness-summary.ps1` проверяет этот operator artifact перед handoff. Live VPS/staging evidence все еще требуется.
