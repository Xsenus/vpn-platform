...
- Browser console smoke: `184/184`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-admin-concurrent-busy-resource-owner`, версия `0.624.0`; все admin mutation indicators используют multi-owner resource busy-state, поэтому завершение независимой команды не разблокирует незавершённую форму или строку. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
