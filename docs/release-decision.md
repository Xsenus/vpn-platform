...
- Browser console smoke: `182/182`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-admin-managed-config-resource-owner`, версия `0.623.0`; tariff/release/FAQ/scenario entity commands, global site-content writes и Telegram settings save/test используют общие resource owners, поэтому sibling managed commands не пересекают state mutations. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
