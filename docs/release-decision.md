...
- Browser console smoke: `156/156`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-public-checkout-session-boundary`, версия `0.605.0`; public checkout result, order/payment identifiers, redirect URL и order-bearing diagnostics очищаются на logout/session rejection/password reset boundary. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
