...
- Browser console smoke: `128/128`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-app-version-session-boundary`, версия `0.591.0`; cabinet latest/history requests привязаны к token/user generation, stale completions после logout/login не блокируют и не перезаписывают новую сессию. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
