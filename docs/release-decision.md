...
- Latest "Что нового": `2026-07-02-dotfiles-utf8-guard`, версия `0.438.0`; DocumentationEncodingTests now include tracked dotfiles such as `.dockerignore`, `.editorconfig`, `.gitattributes` and `.gitignore` in strict UTF-8 and mojibake checks. Production readiness gate и full live VPS/staging evidence все еще требуются.
