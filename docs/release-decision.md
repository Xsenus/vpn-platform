...
- VPN access enable/sync/reset preserves caller cancellation semantics and durable history/audit; enable/reset uncertainty is represented by `SyncRequired` instead of a false confirmed state; cabinet/admin invalidate cached QR before refresh or local blocking.
