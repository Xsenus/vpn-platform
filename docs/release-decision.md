...
- Browser console smoke: `138/138`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-payment-provider-retry`, версия `0.596.0`; cabinet provider discovery имеет одну token-scoped attempt, единый alert и generation-guarded retry без session reset. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
