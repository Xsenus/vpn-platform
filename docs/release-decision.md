...
- Browser console smoke: `176/176`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-admin-subscription-access-resource-owner`, версия `0.620.0`; subscription lifecycle и direct VPN access commands используют общие identity-safe resource owners, поэтому sibling command не пересекает provider-state mutation, а cached QR очищается. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
