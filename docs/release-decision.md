...
- Browser console smoke: `186/186`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-payment-provider-lock`, версия `0.625.0`; cabinet блокирует выбор payment provider до завершения renewal/retry mutation, поэтому видимое значение не расходится с отправленным snapshot. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
