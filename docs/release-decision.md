...
- Latest "Что нового": `2026-06-22-local-admin-bootstrap-smoke-max-duration`, версия `0.253.0`; `scripts/local-admin-vps-bootstrap-smoke.ps1` принимает `MaxEvidenceChainMinutes` и передает его в `scripts/admin-vps-bootstrap-smoke.ps1`, а `scripts/admin-vps-smoke.ps1`/`scripts/admin-vps-bootstrap-smoke.ps1` прокидывают этот лимит в smoke/bootstrap evidence validators. `scripts/validate-admin-vps-smoke-evidence.ps1` включает fail-closed проверку уникальности report ids, префиксов `admin-vps-smoke-preflight-`/`admin-vps-smoke-`, timestamp-формата `yyyyMMdd-HHmmss`, связи timestamp с `preflight.generatedAt`/`smoke.startedAt`, expected SHA256 guard, duration/status/chronology summary и `MaxEvidenceChainMinutes` guard для linked smoke evidence, а `scripts/validate-admin-vps-bootstrap-smoke-evidence.ps1` требует уникальные readiness/bootstrap/preflight/smoke report ids, ожидаемые префиксы, timestamp-формат, связь timestamp с `generatedAt`/`startedAt`, chronology summary и `MaxEvidenceChainMinutes` guard для readiness/bootstrap/preflight/smoke bundle; production readiness gate и live VPS/staging evidence все еще требуются.
