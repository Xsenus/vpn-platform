...
- Browser console smoke: `200/200`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-public-checkout-session-expiry`, версия `0.631.0`; public checkout завершает expired session до claim либо после одного legacy backend rejection без payment init. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
