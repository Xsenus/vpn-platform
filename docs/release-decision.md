...
- Browser console smoke: `136/136`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-app-version-latest-recovery`, версия `0.595.0`; manual latest failure/empty показывает controlled modal, generation-guarded retry и не запускает history до current release. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
