...
- Browser console smoke: `146/146`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-restored-data-boundary`, версия `0.600.0`; failed restored-session load сохраняет cabinet tokens, скрывает false aggregate/private empty states и восстанавливает фактические данные explicit retry без refresh. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
