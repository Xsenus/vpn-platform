...
- Browser console smoke: `154/154`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-section-load-boundary`, версия `0.604.0`; partial cabinet failures сохраняют профиль и независимые здоровые области, а неподтверждённые значения и действия заменяются локальным recovery-state. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
