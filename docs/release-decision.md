...
- Latest "Что нового": `2026-06-22-admin-vps-bootstrap-smoke-env-max-duration`, версия `0.256.0`; `scripts/admin-vps-bootstrap-smoke.ps1` использует `ADMIN_VPS_SMOKE_MAX_EVIDENCE_CHAIN_MINUTES` как default для `MaxEvidenceChainMinutes`, а `scripts/test-admin-vps-bootstrap-smoke-wrapper.ps1` проверяет fail-fast сценарии `bad-max-evidence-chain-minutes` и `bad-env-max-evidence-chain-minutes` до запуска readiness/smoke artifacts. `scripts/admin-vps-smoke.ps1`/`scripts/admin-vps-bootstrap-smoke.ps1` прокидывают этот лимит в smoke/bootstrap evidence validators; production readiness gate и live VPS/staging evidence все еще требуются.
