...
- Latest "Что нового": `2026-06-23-admin-vps-readiness-provider-normalization`, версия `0.274.0`; standalone admin VPS bootstrap readiness now canonicalizes `Postgres`/`Sqlite` provider values before writing sanitized evidence. Production readiness gate и live VPS/staging evidence все еще требуются.
