...
- Latest "Что нового": `2026-08-13-admin-user-overview-read-boundary`, версия `0.685.0`; users list и user overview orders/payments/support ограничены до materialization, role filter использует точный CSV-токен. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
