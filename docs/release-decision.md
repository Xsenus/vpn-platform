...
- Browser console smoke: `188/188`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-renewal-provider-availability`, версия `0.626.0`; сохранённый renewal payment retry блокируется без доступного provider и не превращается в активную no-op команду. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
