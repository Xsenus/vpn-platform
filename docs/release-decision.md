...
- Latest "Что нового": `2026-06-22-admin-vps-bootstrap-evidence-chronology-summary`, версия `0.249.0`; `scripts/validate-admin-vps-smoke-evidence.ps1` включает fail-closed проверку уникальности report ids, префиксов `admin-vps-smoke-preflight-`/`admin-vps-smoke-`, timestamp-формата `yyyyMMdd-HHmmss`, связи timestamp с `preflight.generatedAt`/`smoke.startedAt`, expected SHA256 guard и duration/status summary для linked smoke evidence, а `scripts/validate-admin-vps-bootstrap-smoke-evidence.ps1` требует уникальные readiness/bootstrap/preflight/smoke report ids, ожидаемые префиксы, timestamp-формат, связь timestamp с `generatedAt`/`startedAt` и chronology summary для readiness/bootstrap/preflight/smoke bundle; production readiness gate и live VPS/staging evidence все еще требуются.
