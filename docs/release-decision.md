...
- Browser console smoke: `172/172`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-app-version-seen-single-flight`, версия `0.618.0`; server acknowledgement закрытия cabinet app-version имеет session/release-scoped Promise owner, поэтому duplicate close не повторяет `mark-seen` POST, а transient failure допускает следующую попытку. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
