...
- Latest "Что нового": `2026-07-02-production-env-normalizer-cleanup`, версия `0.399.0`; production env normalizer regression removes default local fixtures after ordinary runs while custom `-OutputDirectory` evidence remains available for explicit debugging. Production readiness gate и full live VPS/staging evidence все еще требуются.
