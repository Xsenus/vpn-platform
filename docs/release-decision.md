...
- Latest "Что нового": `2026-06-22-admin-vps-smoke-evidence-report-id-timestamp`, версия `0.246.0`; `scripts/validate-admin-vps-smoke-evidence.ps1` включает fail-closed проверку уникальности report ids, префиксов `admin-vps-smoke-preflight-`/`admin-vps-smoke-`, timestamp-формата `yyyyMMdd-HHmmss`, expected SHA256 guard и duration/status summary для linked smoke evidence, а `scripts/validate-admin-vps-bootstrap-smoke-evidence.ps1` требует уникальные readiness/bootstrap/preflight/smoke report ids, ожидаемые префиксы и timestamp-формат для readiness/bootstrap/preflight/smoke bundle; production readiness gate и live VPS/staging evidence все еще требуются.
