...
- Browser console smoke: `130/130`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-app-version-identity-boundary`, версия `0.592.0`; app-version latest/dismissal принадлежит подтверждённому profile ID, а token-only hydration не открывает anonymous modal. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
