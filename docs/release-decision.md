...
- Latest "Что нового": `2026-06-22-admin-vps-bootstrap-readiness-preflight-timing-link`, версия `0.220.0`; `scripts/validate-admin-vps-bootstrap-smoke-report.ps1 -RequirePassed` теперь требует порядок readiness `generatedAt` -> linked preflight `generatedAt`, а regression harness покрывает `preflight-generated-before-readiness`; readiness smoke/preflight path checks из `0.219.0` остаются в силе; production readiness gate и live VPS/staging evidence все еще требуются.
