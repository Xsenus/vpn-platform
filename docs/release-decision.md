...
- Browser console smoke: `148/148`; responsive all-screens: `6/6` на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints; local WebP decode и representative screenshots проверены.
...
- Latest "Что нового": `2026-08-11-cabinet-auth-data-boundary`, версия `0.601.0`; unauthenticated cabinet скрывает false aggregate/private states до профиля, а успешный login показывает фактические данные без reload. Real VPS/SSH/Ansible, provider/Telegram/SMTP delivery и полный staging/production-like 3x-ui evidence все еще требуются.
