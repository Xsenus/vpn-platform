...
- Password reset использует durable email queue и SMTP adapter; reset-код хранится в delivery payload только через `ISecretProtector`. Production startup fail-closed требует SMTP, кроме явно подтверждённого degraded mode, где reset endpoints и email delivery отключены. Реальная доставка остается внешней staging/VPS-проверкой.
