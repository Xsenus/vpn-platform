...
VPS production smoke generator release guard removes its autogenerated empty `tmp` directory after a local run.

VPS production smoke reports include `smokeReportPath`; the validator resolves that field and requires it to match the actual `-ReportPath`, so standalone evidence cannot silently point at a different JSON file.

Regression check:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\test-vps-production-smoke-report-self-link-guard.ps1
```
...
VPS production smoke latest release guard removes its autogenerated `tmp/vps-production-smoke-stale-release-guard.json` report and empty `tmp` directory after a local run.
