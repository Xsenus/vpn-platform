...
Default production readiness assertion CI regression run removes its autogenerated `tmp/production-readiness-assertion-ci-regression-test` output after a successful non-JSON local run. Explicit `-OutputDirectory` and `-WriteJson` runs keep artifacts for validators, CI wrappers and evidence handoff.
...
Harness проверяет happy path aggregate guard, затем портит копию workflow и ожидает ошибки для `missing-aggregate-ci-step-guard-command`, `missing-aggregate-ci-step-validator`, `missing-readiness-guard-step`, `missing-readiness-assertion-log-artifact`, `missing-production-evidence-result-artifact` и `missing-if-no-files-found-error`.
...
Regression guard для Markdown files в package index доказывает, что package-validator не принимает Markdown-пару без artifact file details:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\test-production-evidence-handoff-package-markdown-files-guard.ps1
```
...
Default long-path regression run removes its autogenerated `tmp/production-evidence-handoff-package-archive-long-release-id-path-regression-test` output after a successful non-JSON local run. Explicit `-OutputDirectory` and `-WriteJson` runs keep artifacts for validators, CI wrappers and evidence handoff.
...
Package archive validator сверяет SHA256 внешнего ZIP, запрещает вложенные и неожиданные entries, проверяет обязательные `production-evidence.zip`, receipt, checklist, package index и `SHA256SUMS.txt`, временно извлекает package и повторно запускает `validate-production-evidence-handoff-package.ps1`. В режиме `-RequireProductionReady` archive validator сначала сверяет `releaseId` package index с latest active release из `backend/src/VpnPlatform.Api/AppReleases/releases.json`, затем строгая проверка доходит до production-ready handoff на уровне package validator.

Latest-release guard для package archive:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\test-production-evidence-handoff-package-archive-latest-release-guard.ps1
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\test-production-evidence-handoff-package-archive-dotdot-entry-guard.ps1
```

Dotdot-entry guard удаляет созданный ZIP и пустой `tmp` после выполнения, чтобы локальный regression run не оставлял временные artifacts.

Regression guard для dot entries доказывает, что archive-validator не принимает ZIP с entry именем `.`:
...
Whitespace-entry guard удаляет созданный ZIP и пустой `tmp` после выполнения, чтобы локальный regression run не оставлял временные artifacts.
...
Default flow run removes its autogenerated `tmp/production-evidence-handoff-package-archive-flow-test` output after a successful non-JSON local run. Explicit `-OutputDirectory` and `-WriteJson` runs keep artifacts for validators, CI wrappers and evidence handoff.
...
Default CI regression run removes its autogenerated `tmp/production-evidence-handoff-package-archive-ci-regression-test` output after a successful non-JSON local run. Explicit `-OutputDirectory` and `-WriteJson` runs keep artifacts for validators, CI upload and evidence handoff.
