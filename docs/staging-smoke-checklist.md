...
## Partial live result 2026-08-24

Sanitized evidence сохранен в `docs/evidence/staging-smoke-2026-08-24-partial.json` и проходит структурный validator без acceptance-флага:

- `8/18` checks passed: live/ready health, public/cabinet/admin SPA rendering, tariffs, browser console и отсутствие чувствительных значений в отчете;
- `2/18` failed: target release deploy остановлен mail preflight до upload, а все 7 публичных payment providers остались в `Sandbox/Unknown`;
- `8/18` blocked: admin login, checkout/payment/confirmation/subscription/VPN access/support и внешний rotation record;
- `-RequireAllPassed` ожидаемо остается красным, поэтому `P9-TST-007`, `P11-ACC-002` и live `P0-*` не закрыты.
