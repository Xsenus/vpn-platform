...
- browser console smoke: 196/196; responsive all-screens: 6/6 на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints;
...
- latest "Что нового": `2026-08-11-public-expired-partial-checkout`, версия `0.629.0`; публичный partial checkout проверяет status/`expiresAt` до payment init и направляет просроченный заказ к созданию нового.
- roadmap progress: `642/662` closed, readiness `97.0%`, `20` remaining, `19` open, `1` in progress and `0` blocked.
