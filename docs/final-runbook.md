...
- browser console smoke: 200/200; responsive all-screens: 6/6 на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints;
...
- latest "Что нового": `2026-08-11-public-checkout-session-expiry`, версия `0.631.0`; persisted session expiry блокирует claim до API, legacy backend expiry завершает flow без payment init.
- roadmap progress: `644/664` closed, readiness `97.0%`, `20` remaining, `19` open, `1` in progress and `0` blocked.
