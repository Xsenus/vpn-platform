...
- browser console smoke: 212/212; responsive all-screens: 6/6 на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints;
...
- latest "Что нового": `2026-08-12-public-pending-checkout-live-expiry`, версия `0.634.0`; общий current-time expiry timer обновляет anonymous checkout-session, partial order и последнюю покупку без действия пользователя.
- roadmap progress: `647/667` closed, readiness `97.0%`, `20` remaining, `19` open, `1` in progress and `0` blocked.
