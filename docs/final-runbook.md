...
- backend full suite: 1574/1574;
- frontend tests: 189/189;
- API build: OK;
- frontend typecheck/build: OK;
- fresh local SQLite smoke: OK;
- browser console smoke: 280/280; responsive all-screens: 14/14 на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints;
...
- VPN access lifecycle: enable/sync/reset пробрасывают caller cancellation после durable history/audit; enable/reset uncertainty сохраняется как `SyncRequired` для ручной сверки; cabinet/admin очищают cached QR до повторной проверки или local blocker, а API-клиент переводит известные promo-фразы и скрывает остальные technical/English diagnostics и native network errors за русским fallback.
...
- admin production bundle budget: `5` JS chunks, largest `273472`, total raw `582209/582656`, gzip `154904/155648`.
...
- latest "Что нового": `2026-08-15-payment-provider-concurrency-integrity`, версия `0.739.0`; payment-provider accounts защищены persisted revision/no-op границами, а delayed conflict сохраняет более новый draft.
