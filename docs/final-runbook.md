...
- latest "Что нового": `2026-08-12-payment-init-order-provider-snapshot`, версия `0.655.0`; init отклоняет provider mismatch/empty provider ID, а Telegram блокирует смену после первой попытки.
