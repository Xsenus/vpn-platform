...
- production VPN panel smoke: GitHub run `33365986521` подключил deployed API к реальному 3x-ui, подтвердил health-check, secret-redacted admin GET, sync `1` active inbound и production-ноду `Ready/Healthy`; sanitized report `docs/evidence/vpn-live-smoke-2026-08-31.json` содержит `3 passed / 6 blocked`.
