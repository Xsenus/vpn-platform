...
- VPN access lifecycle: enable/sync/reset пробрасывают caller cancellation после durable history/audit; enable/reset uncertainty сохраняется как `SyncRequired` для ручной сверки; cabinet/admin очищают cached QR до повторной проверки или local blocker, а API-клиент скрывает технические `snake_case` ошибки и пустые сообщения из `error`, `message` и plain-text payload за контекстным fallback.
...
- latest "Что нового": `2026-08-10-api-error-payload-hardening`, версия `0.587.0`; `error` и `message` machine codes, whitespace payload и неизвестный auth code проверены с безопасным fallback.
