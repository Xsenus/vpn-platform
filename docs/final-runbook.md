...
- browser console smoke: 148/148; responsive all-screens: 6/6 на 25 viewport-конфигурациях `305x568..2560x1440` с точными парами `N/N+1` для всех CSS-breakpoints;
...
- latest "Что нового": `2026-08-11-cabinet-auth-data-boundary`, версия `0.601.0`; unauthenticated cabinet показывает только auth/help/recovery поверхности, а приватные метрики и коллекции появляются после подтверждённой загрузки профиля.
- roadmap progress: `614/634` closed, readiness `96.8%`, `20` remaining, `19` open, `1` in progress and `0` blocked.
