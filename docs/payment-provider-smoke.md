...
Payment provider smoke generator release guard removes its autogenerated empty `tmp` directory after a local run.
...
Payment provider smoke latest release guard removes its autogenerated `tmp/payment-provider-smoke-stale-release-guard.json` report and empty `tmp` directory after a local run.

Payment provider smoke reports include `smokeReportPath`; the validator resolves that field and requires it to match the actual `-ReportPath`, so standalone evidence cannot silently point at a different provider smoke JSON file.

```powershell
powershell -ExecutionPolicy Bypass -File scripts\test-payment-provider-smoke-report-self-link-guard.ps1
```
