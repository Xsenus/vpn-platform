...
Regression harness также покрывает `mismatched-preflight-report-path`: preflight/smoke evidence не должно принимать preflight report, если поле `preflightReportPath` указывает не на фактически проверяемый preflight JSON. Valid-сценарий проверяет, что success summary содержит `preflightReportPath` и `sectionsContractPath`.
