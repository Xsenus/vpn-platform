...
Единый wrapper `scripts/admin-vps-smoke.ps1` дополнительно запускает preflight с `-RequireRemoteReleaseMatch`: preflight логинится через `/api/auth/login`, держит bearer-токен только в памяти и перед browser smoke требует, чтобы `/api/app-version/latest` на VPS вернул тот же `releaseId`, что и локальный smoke. Это останавливает stale deploy до кликов по старой админке, а отчет сохраняет только sanitized `remoteReleaseId`, `remoteReleaseMatched`, `remoteReleaseStatus` (`not-required`, `matched`, `mismatch`, `unavailable`) и `remoteReleaseMessage` без токенов и паролей. Console summary также печатает безопасные строки `Remote release status`, `Remote release expected` и `Remote release actual`.
