...
Regression harness также покрывает `mismatched-preflight-report-path`, `mismatched-expected-preflight-sha256`, `smoke-completed-before-started` и valid expected SHA256: preflight/smoke evidence не должно принимать preflight report, если поле `preflightReportPath` указывает не на фактически проверяемый preflight JSON, не должно принимать smoke report с `completedAt` раньше `startedAt` и должно fail-closed отклонять bundle при неверном expected SHA256. Valid-сценарий проверяет, что success summary содержит `adminEmail`, `operator`, `preflightReportId`, `smokeReportId`, `preflightReportPath`, `sectionsContractPath`, `preflightReportSha256`, `smokeReportSha256`, `preflightGeneratedAt`, `smokeStartedAt`, `smokeCompletedAt`, `preflightToSmokeSeconds`, `smokeDurationSeconds`, а валидатор принимает `ExpectedPreflightReportSha256`/`ExpectedSmokeReportSha256`.
