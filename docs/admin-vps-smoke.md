...
Admin VPS smoke sections contract regression removes its autogenerated fixtures and empty `tmp` directory after local runs.
...
Admin VPS smoke release guard harnesses remove their autogenerated reports and empty `tmp` directory after local runs.
...
Обычный запуск admin VPS smoke evidence validator regression берет latest active release из `backend/src/VpnPlatform.Api/AppReleases/releases.json` и удаляет default output directory вместе с пустым `tmp`; `-KeepArtifacts` сохраняет regression evidence для локального расследования.
...
Обычный запуск `scripts/local-admin-vps-bootstrap-smoke.ps1` удаляет временную SQLite-БД, отчеты, логи и пустой `tmp`; `-KeepArtifacts` сохраняет `tmp/local-admin-vps-bootstrap-smoke` для локального расследования.
...
Admin VPS bootstrap release guard harnesses remove their autogenerated reports and empty `tmp` directory after local runs.
Admin VPS bootstrap smoke readiness regression removes its autogenerated reports and empty `tmp` directory after local runs.
