...
Единый wrapper `scripts/admin-vps-smoke.ps1` дополнительно запускает preflight с `-RequireRemoteReleaseMatch`: preflight логинится через `/api/auth/login`, держит bearer-токен только в памяти и перед browser smoke требует, чтобы `/api/app-version/latest` на VPS вернул тот же `releaseId`, что и локальный smoke. Это останавливает stale deploy до кликов по старой админке, а отчет сохраняет только sanitized `remoteReleaseId`, `remoteReleaseMatched`, `remoteReleaseStatus` (`not-required`, `matched`, `mismatch`, `unavailable`), `remoteReleaseMessage`, `checkCount`, `passedCheckCount`, `failedCheckCount` и `failedChecks` без токенов и паролей. Console summary также печатает безопасные строки `Preflight report id`, `Remote release status`, `Remote release message`, `Remote release expected`, `Remote release actual`, `Check count`, `Passed checks`, `Failed check count` и `Failed checks`.
...
The wrapper normalizes identity and workspace path inputs before handing them to downstream scripts: `ApiBaseUrl`, `AdminWebUrl`, `AdminEmail` and `FrontendPath` are trimmed once and the same values are used in console output, preflight evidence and browser smoke args.
...
Acceptance mode requires bootstrap readiness `releaseId` to match the latest active release from `backend/src/VpnPlatform.Api/AppReleases/releases.json`.
...
`scripts/test-admin-vps-bootstrap-smoke-latest-release-guard.ps1` covers stale `releaseId` rejection for both bootstrap readiness and final bootstrap smoke reports in acceptance mode.

`scripts/test-admin-vps-bootstrap-smoke-evidence-latest-release-guard.ps1` covers stale `releaseId` rejection for the paired bootstrap readiness/smoke evidence chain.
