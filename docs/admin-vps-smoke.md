...
Regression harness также покрывает `mismatched-preflight-report-path`, `mismatched-expected-preflight-sha256` и valid expected SHA256: preflight/smoke evidence не должно принимать preflight report, если поле `preflightReportPath` указывает не на фактически проверяемый preflight JSON, и должно fail-closed отклонять bundle при неверном expected SHA256. Valid-сценарий проверяет, что success summary содержит `preflightReportPath`, `sectionsContractPath`, `preflightReportSha256`, `smokeReportSha256`, а валидатор принимает `ExpectedPreflightReportSha256`/`ExpectedSmokeReportSha256`.
