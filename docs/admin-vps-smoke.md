...
Если нужно в одном проходе восстановить production admin-аккаунт и сразу доказать вход в `/admin/`, используйте wrapper `scripts/admin-vps-bootstrap-smoke.ps1`. Он вычисляет release id один раз для readiness/smoke/bootstrap evidence, явно fail-fast отклоняет нечисловой или неположительный CLI/env `MaxEvidenceChainMinutes` и невалидные `ApiBaseUrl`/`AdminWebUrl` до readiness/bootstrap/smoke artifacts, запускает `scripts/admin-bootstrap.ps1`, передает пароль в smoke только через process env `ADMIN_VPS_SMOKE_ADMIN_PASSWORD`, затем запускает `scripts/admin-vps-smoke.ps1 -AccountBootstrapChecked` и прокидывает `MaxEvidenceChainMinutes` в smoke/bootstrap evidence validators.
