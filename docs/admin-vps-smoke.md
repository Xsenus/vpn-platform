...
Если нужно в одном проходе восстановить production admin-аккаунт и сразу доказать вход в `/admin/`, используйте wrapper `scripts/admin-vps-bootstrap-smoke.ps1`. Он вычисляет release id и `operatorValue` один раз для readiness/smoke/bootstrap evidence, явно fail-fast отклоняет нечисловой или неположительный CLI/env `MaxEvidenceChainMinutes`, невалидные `ApiBaseUrl`/`AdminWebUrl`, невалидный `AdminEmail` и совпадающие report paths до readiness/bootstrap/smoke artifacts, использует `manual-operator` при пустом `Operator`, запускает `scripts/admin-bootstrap.ps1`, передает пароль в smoke только через process env `ADMIN_VPS_SMOKE_ADMIN_PASSWORD`, затем запускает `scripts/admin-vps-smoke.ps1 -AccountBootstrapChecked` и прокидывает `MaxEvidenceChainMinutes` в smoke/bootstrap evidence validators.
...
Regression includes `too-high-env-max-evidence-chain-minutes`, `bad-api-url`, `bad-admin-web-url`, `bad-admin-email`, `same-report-paths` and `dry-run-default-operator`: these scenarios must fail before readiness, bootstrap reset, preflight and smoke artifacts.
