...
Временный статус работы с roadmap: активная локальная доработка синхронизирована до `2026-08-10-admin-payment-refund-lifecycle-fix`, версия `0.563.0`. Roadmap остается staging-ready baseline, не production-ready: закрыто `576/596` проверяемых пунктов, готовность `96.6%`, осталось `20`, открыто `19`, в работе `1`, блокеров `[!]` нет. Дальше нельзя закрывать `STATE-011`, `STATE-012`, `STATE-013`, `P0-ADMIN-001`, `P0-ADMIN-002`, `P0-VPN-*`, `P0-PAY-*`, `P9-TST-007` и `P11-ACC-002` без реального VPS/staging/live evidence.
...
- [x] `P11-ACC-236` Сделать публикацию provisioning retry атомарной. 2026-08-05.
  - Что сделать: retry run не должен становиться claimable как `PrecheckQueued`/`DeployQueued` до отдельного коммита `Retrying`, иначе worker другого инстанса может начать или завершить run, а API затем перезапишет актуальный статус.
  - Что сделано: `QueueCoreAsync` создает retry сразу со статусом `Retrying`, согласованными node/step/log/audit данными и сохраняет весь enqueue одним `SaveChanges`; coordinator уже атомарно claim-ит `Retrying` в `Prechecking`/`Deploying`.
  - Доказательство: backend `1060/1060`, targeted provisioning/coordinator `39/39`, SQLite `SaveChangesInterceptor` подтверждает один commit на retry и persisted `Retrying` step до возврата API, sandbox worker E2E выполняет retry до deploy, frontend `84/84`, typecheck/build, Playwright desktop/mobile/all-screens responsive `16/16`, API/TelegramBot Release builds `0` warnings/`0` errors, EF pending model changes отсутствуют, dependency audit `0 vulnerabilities`, secret scan `639` files/`0` findings, latest "Что нового" `2026-08-05-provisioning-retry-atomic-queue`, версия `0.513.0`. External production evidence остается открытым.
- [x] `P11-ACC-237` Защитить provisioning cancel от concurrent worker claim. 2026-08-05.
  - Что сделать: cancel queued run не должен перезаписывать `Prechecking`/`Deploying`, если другой worker claim-нул задачу после API-read; node нельзя возвращать в `New`, пока внешнее выполнение уже началось.
  - Что сделано: cancel читает snapshot без tracking, в relational transaction выполняет conditional update по `status + UpdatedAt`, сохраняет node и audit в том же commit и поддерживает монотонную version при clock skew. Проигранная гонка возвращает HTTP `409` без partial node/audit mutations, а admin UI перечитывает актуальные runs.
  - Доказательство: backend `1061/1061`, targeted provisioning/coordinator `40/40`, deterministic file-backed SQLite transaction interceptor claim race через реальный controller, отсутствие ложного cancel audit/node mutation, frontend conflict-refresh regression в существующем `84/84`, typecheck/build, Playwright desktop/mobile/all-screens responsive `16/16`, API/TelegramBot Release builds `0` warnings/`0` errors, EF pending model changes отсутствуют, dependency audit `0 vulnerabilities`, secret scan `639` files/`0` findings, latest "Что нового" `2026-08-05-provisioning-cancel-claim-boundary`, версия `0.514.0`. External production evidence остается открытым.
- [x] `P11-ACC-238` Сделать claim checkout-сессии атомарным. 2026-08-05.
  - Что сделать: два пользователя не должны одновременно создать заказы из одной checkout-сессии до раздельного сохранения её владельца; повторный запрос того же пользователя должен получить winning order, а terminal completed state нельзя переписывать в expired.
  - Что сделано: relational claim сначала условно резервирует snapshot session как `claiming`, затем создаёт order и публикует `claimed + OrderId` в одной transaction. Проигравший запрос перечитывает committed winner: тому же user возвращается тот же order, другому user — controlled rejection без второго заказа. Связанная completed-сессия остаётся идемпотентной после token expiry.
  - Доказательство: backend `1065/1065`, targeted checkout/order/payment `21/21`, deterministic file-backed SQLite transaction interceptor races для same/different user, один persisted order и согласованные session owner/order/status, transaction rollback reservation при order failure, fresh local SQLite checkout/payment/subscription/VPN access smoke, frontend `84/84`, typecheck/build, Playwright desktop/mobile/all-screens responsive `16/16`, API/TelegramBot Release builds `0` warnings/`0` errors, EF pending model changes отсутствуют, dependency audit `0 vulnerabilities`, secret scan `639` files/`0` findings, latest "Что нового" `2026-08-05-checkout-claim-atomicity`, версия `0.515.0`. External production evidence остаётся открытым.
- [x] `P11-ACC-239` Завершить fail-closed жизненный цикл промокодов. 2026-08-05.
  - Что сделать: недействительный промокод не должен молча игнорироваться; период, tariff/channel allow-list, discount configuration и redemption limits должны проверяться до checkout/order, а бесплатные дни должны сохраняться в контракте заказа и применяться при активации и renewal.
  - Что сделано: checkout prevalidation и `OrderService` централизованно проверяют промокод; relational conditional version update сериализует limit slot, stale pending order освобождает слот, intent identity учитывает `PromoCodeId`. Заказ сохраняет promo/free-days snapshot, `SubscriptionService` применяет его к новой и продленной подписке с безопасной date-range проверкой. API возвращает controlled `409`, public UI показывает русскую ошибку.
  - Доказательство: backend `1081/1081`, targeted promo/checkout/order/subscription `40/40`, deterministic file-backed SQLite final-slot race через реальный controller, stale expiration conflict/idempotent/allow-list/free-days/date-range regressions, fresh local SQLite checkout/payment/subscription/VPN access smoke, frontend `85/85`, typecheck/build, Playwright desktop/mobile/all-screens responsive `16/16`, API/TelegramBot Release builds `0` warnings/`0` errors, EF pending model changes отсутствуют, dependency audit `0 vulnerabilities`, secret scan `640` files/`0` findings, latest "Что нового" `2026-08-05-promo-lifecycle-integrity`, версия `0.516.0`. External production evidence остаётся открытым.
- [x] `P11-ACC-240` Завершить жизненный цикл реферальных начислений. 2026-08-05.
  - Что сделать: регистрация по реферальному коду должна сохранять связь, завершенная подходящая покупка должна идемпотентно создавать начисления через durable outbox, пользовательский API не должен раскрывать source/metadata, а администратор должен управлять валидными программами без ручного JSON.
  - Что сделано: регистрация атомарно создаёт `ReferralRelationship`; `SubscriptionService` публикует `ReferralRewardRequested`, локальный sink материализует deterministic `RewardLedger` для пригласившего и приглашённого с проверками периода, первой покупки, суммы, канала, тарифа и promo stacking. Cabinet DTO redacted; admin GET/POST/PATCH валидируют и отображают программы, условия и журнал начислений; public/cabinet принимают referral code.
  - Доказательство: backend `1087/1087`, targeted auth/referral/outbox/subscription `34/34`, SQLite materialization/idempotency/dead-letter/redaction tests, frontend `89/89`, typecheck/build, Playwright desktop/mobile/all-screens responsive `16/16`, fresh local SQLite smoke, API/TelegramBot Release builds `0` warnings/`0` errors, EF pending model changes отсутствуют, dependency audit `0 vulnerabilities`, secret scan `642` files/`0` findings и strict UTF-8 guard. Latest "Что нового" `2026-08-05-referral-reward-lifecycle`, версия `0.517.0`. External production evidence остаётся открытым.
- [x] `P11-ACC-241` Завершить durable lifecycle email-уведомлений. 2026-08-05.
  - Что сделать: pending `NotificationDelivery` должна реально обрабатываться worker-ом; password reset обязан доставлять рабочий код без plaintext persistence, повторов без lease и утечки payload в admin API.
  - Что сделано: добавлены conditional DB claim, stale lease recovery, exponential backoff, terminal failure и outbox deduplication; SMTP adapter включается только явным `Email:Mode=Smtp`, production startup fail-closed проверяет настройки. Reset-код защищается `ISecretProtector`; admin «Аудит» показывает маскированную очередь и разрешает redacted retry только для `Failed`.
  - Доказательство: backend `1098/1098`, targeted email/auth/outbox/audit/startup SQLite suite `40/40`, frontend `90/90`, typecheck/build, Playwright desktop/mobile/all-screens responsive `16/16`, fresh local SQLite smoke, migration `20260805092911_EmailNotificationDeliveryLifecycle`, EF pending model changes отсутствуют, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings и strict UTF-8 guard. Реальная SMTP-доставка на staging/VPS не была доступна и остается внешним evidence.
- [x] `P11-ACC-242` Перенести checkout context на доверенную серверную границу. 2026-08-05.
  - Что сделать: public/cabinet payload не должен выбирать доверенный канал или признак первой покупки; public checkout не должен сохранять сессию для неподдерживаемого типа или неготового провайдера, а GUID заказа не должен давать анонимный доступ к пользовательским данным.
  - Что сделано: public endpoint допускает только `NewSubscription/Web`, кабинет всегда использует `Web`, `OrderService` вычисляет `IsFirstPurchase` по завершенным покупкам. Provider readiness проверяется до записи checkout-сессии; анонимный order status закрыт `410 Gone`, клиентские поля удалены из typed API.
  - Доказательство: backend `1103/1103`, targeted checkout/order/payment/referral SQLite suite `55/55`, frontend `90/90`, typecheck/build, Playwright public/cabinet/admin desktop/mobile/all-screens responsive `16/16` на ширинах `305..1920`, fresh local SQLite checkout/payment/subscription/VPN smoke и release timestamp regression guard. Внешние live evidence не требуются для локального boundary fix и остаются открыты отдельными пунктами.
- [x] `P11-ACC-243` Защитить UI оплаты от параллельных запусков и блокировки popup. 2026-08-05.
  - Что сделать: один пользовательский жест не должен создавать несколько checkout-сессий через соседние тарифы или быстрый повторный клик; provider/promo нельзя менять во время запроса, а повторная оплата не должна ложно считаться открытой, если браузер заблокировал асинхронный popup.
  - Что сделано: public checkout использует общий busy-state для всех тарифов и полей плюс синхронный `useRef` guard до перерендера. Cabinet сохраняет результат retry payment и показывает отдельную карточку с явной безопасной ссылкой и копированием вместо `window.open` после `await`.
  - Доказательство: frontend `90/90`, public/cabinet typecheck, production build всех трех приложений, Playwright changed desktop/mobile `4/4`, полный public/cabinet/admin/all-screens/mobile/console responsive suite `16/16`, fresh local SQLite checkout/payment/subscription/VPN smoke, backend `1103/1103`. Внешние live evidence не требуются для локального UI guard и остаются открыты отдельными пунктами.
- [x] `P11-ACC-244` Сделать shared copy-команду честной и устойчивой. 2026-08-05.
  - Что сделать: public/cabinet/admin не должны объявлять «Скопировано», если Clipboard API отсутствует или отклоняет разрешение; ошибка не должна становиться unhandled rejection, повторный клик не должен запускать второй вызов, а feedback не должен менять ширину панели.
  - Что сделано: `tryWriteClipboardText` возвращает проверенный результат, `CopyButton` использует synchronous in-flight guard, busy-state, unmount cleanup и постоянно зарезервированную видимую live-status строку. Удалён неиспользуемый admin helper с ложным success.
  - Доказательство: frontend `91/91`, typecheck/build всех трех приложений, cabinet desktop/mobile Clipboard success/NotAllowedError `2/2`, полный public/cabinet/admin/all-screens/mobile/console responsive suite `16/16` на ширинах `305..1920`, fresh local SQLite checkout/payment/subscription/VPN smoke и backend `1103/1103`. Внешние live evidence остаются открыты отдельными пунктами.
- [x] `P11-ACC-245` Закрыть небезопасные внешние URL оплаты на backend и frontend. 2026-08-05.
  - Что сделать: request return URL, redirect провайдера и сохраненный confirmation URL должны допускать только абсолютные `http/https` адреса без встроенных учетных данных; public/cabinet не должны открывать, показывать или копировать отклоненную ссылку.
  - Что сделано: `PaymentOrchestrator` fail-closed валидирует return URL до provider call, не сохраняет небезопасный provider redirect и не переиспользует legacy confirmation URL. Shared `ExternalLinkActions` применяет тот же protocol/credentials boundary ко всем payment и Telegram links, выводя предупреждение без активной команды.
  - Доказательство: backend `1112/1112`, targeted payment initialization SQLite `17/17`, frontend `92/92`, typecheck/build всех трех приложений, public/cabinet desktop/mobile unsafe-link scenarios `4/4`, полный responsive/console suite `16/16` на ширинах `305..1920` и fresh local SQLite checkout/payment/subscription/VPN smoke. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-246` Изолировать SVG QR-кодов от DOM кабинета и админки. 2026-08-05.
  - Что сделать: API-клиент должен принимать QR только с ожидаемым MIME и ограниченным размером, а frontend не должен вставлять полученный SVG как raw DOM; активная или поврежденная разметка должна отклоняться с понятным сообщением.
  - Что сделано: QR API boundary проверяет `image/svg+xml`, пустой ответ и лимит размера. Shared `QrCodePreview` принимает только пассивный корневой SVG без script, event handlers, внешних ссылок и активных элементов, кодирует его как data image и показывает через `<img>` вместо `dangerouslySetInnerHTML` во всех пяти cabinet/admin местах.
  - Доказательство: frontend `94/94`, typecheck/build всех трех приложений, QR desktop/mobile/admin changed suite `8/8`, полный responsive/console suite `16/16`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-247` Закрыть fail-open ссылки действий production readiness в админке. 2026-08-05.
  - Что сделать: server-driven `actionHref` должен принимать только точный fragment существующего admin-раздела, доступного текущей роли; неизвестный, внешний или исполняемый адрес не должен попадать в активный `<a>`.
  - Что сделано: shared `parseAdminSectionHref` использует allow-list всех admin section id и отклоняет любые значения кроме точного `#section`. Dashboard показывает команду только для разрешенного раздела и формирует канонический fragment; неизвестная ссылка остается обычной диагностикой без перехода.
  - Доказательство: frontend `95/95`, typecheck/build всех трех приложений, admin desktop/mobile changed suite `6/6`, полный responsive/console suite `16/16` на ширинах `305..1920`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-248` Ограничить время всех frontend API-запросов и чтения response body. 2026-08-05.
  - Что сделать: зависший fetch или поток body не должен навсегда удерживать loading/busy-state public, cabinet или admin; timeout должен завершаться контролируемой ошибкой, прерывать transport и очищать timer после любого исхода.
  - Что сделано: общий `ApiClient.fetchWithTimeout` охватывает получение headers и полное чтение JSON/text body, использует `AbortController`, возвращает понятную `ApiClientError` со статусом `408` и очищает timeout/внешний abort listener в `finally`. Все API-методы наследуют единый 30-секундный deadline; тесты могут передать короткий deadline через constructor.
  - Доказательство: frontend `97/97`, включая stalled fetch, stalled SVG body и successful timer cleanup `2/2`, typecheck/build всех трех приложений, полный responsive/console suite `16/16` на ширинах `305..1920`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-249` Проверять формат успешных JSON API-ответов до передачи в UI. 2026-08-05.
  - Что сделать: HTML login/proxy page, пустой body, поврежденный JSON или JSON-примитив с успешным HTTP status не должны проходить как типизированный DTO и приводить к render exception в public, cabinet или admin.
  - Что сделано: общий `ApiClient.request<T>` принимает только непустой объект или массив с `application/json` либо `application/*+json`; неподдерживаемый MIME, пустой body, parse error и `null`/primitive завершаются controlled `ApiClientError` `502`. Ошибочные HTTP-ответы сохраняют прежнюю поддержку JSON и text payload.
  - Доказательство: frontend `98/98`, включая пять transport response cases, typecheck/build всех трех приложений, malformed `200 text/html` public regression на desktop/mobile без `pageerror`, полный responsive/console suite `18/18`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-250` Ограничить размер API response body до полной буферизации в браузере. 2026-08-05.
  - Что сделать: oversized JSON, error payload или QR не должен полностью загружаться в память до проверки лимита; declared и фактический размер должны проверяться по байтам, а transport прекращаться сразу после превышения.
  - Что сделано: общий streaming reader проверяет `Content-Length`, считает фактические bytes и вызывает `ReadableStream.cancel()` при превышении. Typed JSON ограничен `10 MB`, error payload `64 KB`, QR `1 MB`; неверный MIME отклоняется до чтения body. Все нарушения возвращают controlled `ApiClientError` `502` без raw payload.
  - Доказательство: frontend `98/98`, включая declared JSON/error и streamed QR cancellation, typecheck/build всех трех приложений, реальный oversized JSON public regression desktop/mobile `2/2` без `pageerror`, полный responsive/console suite `18/18`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-251` Проверять ожидаемую top-level форму каждого typed JSON API-ответа. 2026-08-05.
  - Что сделать: синтаксически корректный object вместо списка или array вместо DTO не должен проходить TypeScript cast и вызывать `.map/.filter` exception либо скрытую потерю полей уже в UI.
  - Что сделано: все `41` list endpoint используют отдельный `requestArray<T>`, остальные typed endpoints требуют non-array object. Object/array mismatch завершается controlled `ApiClientError` `502` с минимальной диагностикой ожидаемой формы до передачи payload вызывающему коду.
  - Доказательство: frontend `98/98`, включая `{}` вместо tariffs и `[]` вместо dashboard DTO, inventory `41/41` list methods, typecheck/build всех трех приложений, wrong-shape public regression desktop/mobile `2/2` без `pageerror`, полный responsive/console suite `18/18`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-252` Проверять обязательные поля публичных API DTO до рендера. 2026-08-09.
  - Что сделать: синтаксически корректный список с пустым, неполным или несовместимым элементом не должен проходить TypeScript cast и приводить к пустым карточкам, неверному checkout или render exception.
  - Что сделано: публичные тарифы, FAQ, контент главной и способы оплаты получили runtime-валидаторы полного backend-контракта: обязательные строки, числа, флаги, вложенный список features, допустимые активные payment mode и уникальные `id/slug/key/provider`. Поврежденный или дублированный элемент завершается controlled `ApiClientError` `502` без raw payload до передачи в UI.
  - Доказательство: frontend `98/98`, включая malformed DTO для всех четырех публичных коллекций, typecheck/build всех трех приложений, invalid-item tariffs regression desktop/mobile `2/2` без `pageerror`, полный responsive/console suite `18/18`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-253` Проверять обязательные поля API DTO личного кабинета до рендера. 2026-08-09.
  - Что сделать: неполный или несовместимый элемент общего cabinet bootstrap не должен проходить TypeScript cast, оставлять старые пользовательские/VPN-данные или приводить к render exception.
  - Что сделано: профиль, подписки, заказы, платежи, VPN-доступы, реферальные начисления, обращения и сообщения поддержки и Telegram status получили runtime-валидаторы полного backend-контракта с проверкой дат, enum, чисел, nullable-полей и уникальных `id`. Эти же валидаторы применены к ответам создания заказа/обращения, загрузки платежа, ответа в поддержку и отвязки Telegram; нарушение возвращает controlled `ApiClientError` `502` без raw payload.
  - Доказательство: frontend `98/98`, кабинетный malformed bootstrap regression desktop/mobile `2/2` с видимым error-state без старого email/VPN URI и без `pageerror`, полный responsive/console suite `18/18`, typecheck/build всех трех приложений, dependency audit `0 vulnerabilities`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-254` Проверять обязательные поля core API DTO админки до рендера. 2026-08-09.
  - Что сделать: поврежденные session/dashboard/users/commercial DTO не должны проходить TypeScript cast, открывать неверные capabilities, ломать раздел или оставлять ранее загруженные персональные данные.
  - Что сделано: административная сессия и capabilities, dashboard counters/readiness, users, user overview, subscriptions, access credentials, orders и payments получили runtime-валидаторы полного backend-контракта с проверкой вложенных массивов, дат, enum и уникальных `id`. При отсутствии выбранного пользователя его stale overview очищается. Backend overview снова включает обязательный support conversation `revision`.
  - Доказательство: frontend `98/98`, включая malformed admin core DTO, admin desktop/mobile `6/6`, malformed users regression `2/2` без stale карточки и `pageerror`, все admin-разделы/responsive widths `2/2`, полный responsive/console suite `18/18`, backend `1112/1112`, SQLite overview regression, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-255` Проверять обязательные поля finance, audit, notification и support API DTO админки до рендера. 2026-08-09.
  - Что сделать: поврежденные финансовые, audit, notification или support ответы не должны проходить TypeScript cast, оставлять ложную readiness/старые способы оплаты либо ломать read и mutation операции.
  - Что сделано: audit logs, notification deliveries/retry, payment recheck/refund, provider accounts/checks, webhook events, refunds, support conversations/messages/reply/status/note получили runtime-валидаторы фактического backend-контракта. Вложенные capabilities/required fields, даты, enum, суммы, nullable-поля, уникальные `id` и internal-note semantics проверяются до передачи в UI; malformed provider list очищает stale способы оплаты.
  - Доказательство: frontend `99/99`, включая malformed finance/audit/notification/support DTO, admin desktop/mobile `6/6`, malformed provider regression без stale readiness, все admin-разделы/responsive widths `2/2`, полный responsive/console suite `18/18`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-256` Проверять обязательные поля content, reference и app release API DTO до рендера. 2026-08-09.
  - Что сделать: поврежденные тарифы, реферальные данные, релизы, FAQ, контент сайта или сценарии работы не должны проходить TypeScript cast, ломать разделы либо оставлять старые коммерческие данные.
  - Что сделано: read и mutation ответы тарифов, referral programs/rewards, app version latest/history/seen/admin releases, FAQ/overview, site content/readiness/default restore и work scenarios получили runtime-валидаторы фактического backend-контракта. Проверяются обязательные поля, даты, enum, nullable-значения, JSON, уникальные `id`, overview/readiness counts и согласованность current/latest version; malformed tariffs очищают stale список.
  - Доказательство: frontend `100/100`, включая malformed content/reference/app-release DTO, admin desktop/mobile `6/6`, malformed tariffs regression `2/2` без stale карточек и `pageerror`, все admin-разделы/responsive widths `2/2`, полный responsive/console suite `18/18`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-257` Проверять обязательные поля 3x-ui panel API DTO до рендера. 2026-08-09.
  - Что сделать: поврежденные panel, inbound, client, sync или health ответы не должны проходить TypeScript cast, ломать раздел либо восстанавливать детали уже снятой панели поздним ответом.
  - Что сделано: все 19 read и mutation маршрутов 3x-ui panel/inbound/client/sync/health получили runtime-валидаторы фактического backend-контракта. Проверяются обязательные поля, даты, enum, JSON-объекты, nullable-значения, capacity/status связи, уникальные `id`, parent id и default inbound semantics; malformed panel list очищает выбранную панель и детали, а request generation отбрасывает устаревшие ответы.
  - Доказательство: frontend `101/101`, включая malformed DTO всех 19 маршрутов, admin desktop/mobile `6/6` с delayed stale-details regression без старых panel/inbound/client/sync данных и `pageerror`, полный responsive/console suite `18/18`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-258` Проверять обязательные поля infrastructure API DTO админки до рендера. 2026-08-09.
  - Что сделать: поврежденные server, node health, provisioning или Telegram bot ответы не должны проходить TypeScript cast, оставлять старую инфраструктуру/readiness либо сохранять форму редактирования исчезнувшего сервера.
  - Что сделано: все 22 read и mutation маршрута VPN server, node health, provisioning run/detail/command/cancel/support и Telegram settings/check получили runtime-валидаторы фактического backend-контракта. Проверяются обязательные поля, даты, enum, JSON, capacity, mode/risk/live-deploy связи, уникальные `id`, `nodeId/runId`, Telegram mask и readiness semantics; malformed refresh очищает stale server/provisioning/bot состояние.
  - Доказательство: frontend `102/102`, включая malformed DTO всех 22 маршрутов, admin/all-screens desktop/mobile `12/12` с очисткой server edit form, provisioning run и delayed Telegram connection check без stale readiness и `pageerror`, полный responsive/console suite `18/18`, backend `1112/1112`, EF model drift отсутствует, fresh local SQLite checkout/payment/subscription/VPN smoke, dependency audit `0 vulnerabilities`, secret scan `649` files/`0` findings. External live payment/VPS/3x-ui/SMTP evidence остается открытым.
- [x] `P11-ACC-259` Проверять critical-flow mutation DTO по фактическому backend-контракту. 2026-08-09.
...
- [x] `P11-ACC-261` Исполнять межсерверную миграцию подписки вместо вечного planned job. 2026-08-09.
  - Что сделать: созданное из админки задание не должно навсегда оставаться `Planned`; перенос обязан изменить реального 3x-ui клиента и согласованно обновить подписку, credential, узлы, capacity и аудит либо завершиться как `Failed` с диагностикой.
  - Что сделано: endpoint выбирает конкретные готовые node/panel/inbound, создает `Running` job, вызывает компенсируемую миграцию 3x-ui и фиксирует `Completed`/`Failed`. Общая операция миграции атомарно резервирует capacity панели, inbound и узла, освобождает источник, обновляет `Subscription.CurrentServerId`, текущий `AccessCredential.ServerId`/URI и не оставляет ложный UI-статус планирования.
  - Доказательство: SQLite boundary подтверждает полный перенос и согласованные счетчики; targeted X3Ui/admin suite `77/77`; backend `1112/1112`; frontend `104/104`; полный Playwright console/responsive suite `20/20`; typecheck/build, fresh local SQLite smoke, EF model drift, dependency audit `0 vulnerabilities`, secret scan `649/0` и strict UTF-8 guards пройдены. Реальный production-like 3x-ui/VPS evidence остается открытым и этим локальным пунктом не закрывается.
- [x] `P11-ACC-262` Открыть межпанельную миграцию VPN-клиента в админке. 2026-08-09.
...
- [x] `P11-ACC-266` Валидировать pending public checkout перед восстановлением из browser storage. 2026-08-09.
...
- [x] `P11-ACC-268` Восстанавливать сохранённую сессию кабинета через refresh-token без потери локального состояния. 2026-08-09.
...
- [x] `P11-ACC-270` Защитить переписку кабинета от out-of-order ответов и очистить приватные drafts при завершении сессии. 2026-08-09.
  - Что сделать: поздний ответ старого support conversation не должен заменять сообщения выбранного обращения или возвращаться после logout; новый пользователь не должен видеть черновики, reset-token и выбранные связи предыдущей сессии.
  - Что сделано: загрузка сообщений получила request generation поверх session operation, сразу очищает старый thread и принимает результат только для актуального обращения; create/reload сохраняет один владелец запроса, а logout инвалидирует pending support/provider операции и сбрасывает support/auth/password-reset/payment state.
  - Доказательство: frontend `110/110`, cabinet desktop/mobile `14/14`, полный console-responsive Playwright `58/58`; E2E задерживает первый thread, переключает обращение, завершает сессию во время второго запроса и после нового входа подтверждает отсутствие чужих сообщений и всех приватных drafts. Typecheck/build и dependency audit пройдены; внешние evidence-пункты не закрывались.
- [x] `P11-ACC-271` Защитить admin detail views и support actions от out-of-order и post-logout ответов. 2026-08-09.
  - Что сделать: user overview и support messages должны принимать только ответ выбранной записи текущей сессии; status action невыбранного обращения не должна заменять открытый диалог, а смена обращения обязана очищать drafts и показывать честное loading/empty состояние.
  - Что сделано: user/support loaders получили request generation поверх session operation и selected refs; selection немедленно инвалидирует старый запрос и очищает detail state/drafts, mutation reload ограничен текущим thread, а общий action wrapper не возвращает error/busy state после logout.
  - Доказательство: frontend `110/110`, admin desktop/mobile `24/24`, полный console-responsive Playwright `62/62`; E2E задерживает оба первых detail-запроса, переключает пользователя/обращение, меняет статус невыбранного thread и завершает сессию перед поздними ответами. Typecheck/build и dependency audit пройдены; внешние evidence-пункты не закрывались.
- [x] `P11-ACC-272` Защитить admin mutations, reload и form drafts от duplicate и stale completion. 2026-08-09.
  - Что сделать: CRUD/payment/subscription/VPN/provisioning/bot actions должны менять UI только в исходной admin-сессии; повторный submit не должен дублировать API request, а завершение старого save или общего reload не должно стирать более новый черновик.
  - Что сделано: `runAction` получил session-owned context и синхронный in-flight guard; все mutation handlers проверяют operation до setter/reload, full/user/VPN loaders используют request generation, сброс CRUD-форм привязан к отправленному snapshot, а dirty Telegram bot form сохраняется при background reload.
  - Доказательство: frontend `110/110`, admin desktop/mobile `28/28`, полный console-responsive Playwright `66/66`; E2E отправляет два submit события, завершает сессию до delayed response, создаёт новый draft после повторного входа и редактирует форму во время pending save. Backend `1112/1112`, Release build, fresh SQLite, EF drift, audit и secret scan пройдены; внешние evidence-пункты не закрывались.
- [x] `P11-ACC-273` Защитить public/cabinet mutations и drafts от duplicate и stale completion. 2026-08-09.
...
- [x] `P11-ACC-277` Проверить полный CRUD управляемой конфигурации admin-панели на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен выполнять create/edit/delete для тарифов, сценариев, релизов, FAQ и контента сайта, create/edit реферальной программы, disable тарифа и восстановление defaults главной; mutation должна подтверждаться после общего reload, а не только фактом HTTP-запроса.
  - Что сделано: admin mock API хранит state управляемых сущностей и реализует фактические POST/PATCH/PUT/DELETE routes. Новый UI flow работает только через формы, доступные кнопки и confirm-dialog, сверяет payload, повторно загруженные строки и отсутствие overflow на desktop/mobile-admin.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; managed CRUD desktop/mobile `2/2`, полный console-responsive Playwright `80/80`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Внешние VPS/staging/live payment/3x-ui/SMTP evidence не закрывались.
- [x] `P11-ACC-278` Проверить Telegram-привязку и status lifecycle поддержки в кабинете на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен фактически создать безопасный Telegram deep-link, отразить linked-state, отвязать Telegram, закрыть и переоткрыть обращение с корректной optimistic revision; результат должен сохраняться после общего reload.
  - Что сделано: cabinet mock API хранит Telegram/support state и реализует POST link-token, DELETE unlink и PATCH status. Новый UI flow сверяет deep-link, authorization, payload/revision `0 -> 1 -> 2`, уведомления, повторную загрузку, console errors и overflow на desktop/mobile-cabinet.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted cabinet lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `82/82`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальная Telegram bot delivery и внешние VPS/staging/live payment/3x-ui/SMTP evidence не закрывались.
- [x] `P11-ACC-279` Проверить secure lifecycle платежных аккаунтов admin-панели на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен выполнить create/edit/disable/reload/enable/check платежного аккаунта, проверить payload/authorization и доказать, что write-only secret values не возвращаются в DTO и не появляются в DOM.
  - Что сделано: admin mock API хранит provider accounts и реализует POST/PATCH/enabled/check routes. Новый UI flow заполняет реальную форму, проверяет безопасные `hasSecret*` признаки, пустые secret-поля при edit, reload persistence, readiness result, console errors и overflow на desktop/mobile-admin.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted secure provider lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `84/84`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальные provider sandbox/live кабинеты и внешние VPS/staging/3x-ui/Telegram/SMTP evidence не закрывались.
- [x] `P11-ACC-280` Проверить secure lifecycle настроек Telegram-бота admin-панели на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен сохранить mode, URLs, тексты и write-only bot/webhook tokens, выполнить readiness check, подтвердить reload persistence и повторное редактирование без возврата или раскрытия raw tokens.
  - Что сделано: admin mock API хранит Telegram settings и реализует PATCH save/POST check. Новый UI flow использует фактическую форму, сверяет два PATCH payload, authorization, очищенные secret inputs, configured-признаки, reload state, console errors и overflow на desktop/mobile-admin.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted secure Telegram settings lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `86/86`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальный Telegram Bot API/webhook и внешние provider/VPS/staging/3x-ui/SMTP evidence не закрывались.
- [x] `P11-ACC-281` Проверить secure managed lifecycle VPN-инфраструктуры admin-панели на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен выполнить create/reload/edit/status/delete для VPN-сервера, create/reload/edit/status/archive для 3x-ui панели и create/edit/default/disable/enable для inbound-правила; SSH credential и panel passwords должны оставаться write-only.
  - Что сделано: admin mock API хранит server/panel/inbound state и реализует фактические POST/PUT/PATCH/DELETE/mode routes. Новый UI flow работает через формы и confirm-dialog, сверяет payload/authorization, reload persistence, очистку secret inputs, безопасные DTO/DOM, console errors и overflow на desktop/mobile-admin.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted infrastructure lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `88/88`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальные VPS/staging/production-like 3x-ui и внешние provider/Telegram/SMTP evidence не закрывались.
- [x] `P11-ACC-282` Проверить safe validation lifecycle подготовки VPS в admin-панели на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен выполнить health-check, dry-run precheck, validation preparation, deploy, cancel, retry и support escalation, проверить mode/risk/operator warning, payload/authorization и reload persistence без реального SSH/Ansible.
  - Что сделано: admin mock API хранит provisioning runs и реализует server health/precheck/provision, run deploy/cancel/retry/support routes. Новый UI flow проходит `ReadyToDeploy -> DeployQueued -> Cancelled -> Retrying`, использует confirm-dialog и проверяет console errors/overflow на desktop/mobile-admin.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted safe provisioning lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `90/90`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальные VPS/SSH/Ansible, staging/production-like 3x-ui и внешние provider/Telegram/SMTP evidence не закрывались.
- [x] `P11-ACC-283` Проверить lifecycle операций 3x-ui клиента в admin-панели на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен выполнить disable, общий reload, enable, sync и reset-traffic через фактические controls, проверить payload/authorization, confirm-dialog и persisted client state.
  - Что сделано: admin mock API хранит VPN client state и реализует enable/disable/sync/reset-traffic POST routes. Новый UI flow сверяет enable/sync status после каждой операции, reload persistence, console errors и overflow на desktop/mobile-admin.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted 3x-ui client lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `92/92`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальный production-like 3x-ui и внешние VPS/staging/provider/Telegram/SMTP evidence не закрывались.
- [x] `P11-ACC-284` Проверить stateful lifecycle VPN-доступа admin-панели на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен выполнить disable, общий reload, enable, sync и reset-traffic, проверить reason payload/authorization, persisted status/disabledAt/revision и отсутствие terminal access secrets.
  - Что сделано: admin mock API хранит access credential state и реализует enable/disable/sync/reset-traffic POST routes. Новый UI flow подтверждает Disabled после reload, revision `1 -> 5`, confirm-dialog, terminal masking, console errors и overflow на desktop/mobile-admin.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted VPN access lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `94/94`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальные provider/VPS/staging/production-like 3x-ui и внешние payment/Telegram/SMTP evidence не закрывались.
- [x] `P11-ACC-285` Проверить stateful lifecycle подписки admin-панели на desktop/mobile. 2026-08-10.
  - Что сделать: browser E2E должен выполнить activate, extend, sync, block/reload/unblock, migrate/reload и cancel/reload, проверить payload/authorization, сохранение subscription/access state и terminal masking.
  - Что сделано: admin mock API сохраняет status, renewal count, block reason, target server и связанную access revision. Новый UI flow подтверждает все семь action routes, reload persistence после блокировки/миграции/отмены, отзыв доступа, отсутствие provider ID/URI/команд, console errors и overflow на desktop/mobile-admin.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted subscription lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `96/96`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальные provider/VPS/staging/production-like 3x-ui и внешние payment/Telegram/SMTP evidence не закрывались.
- [x] `P11-ACC-286` Исправить и проверить stateful lifecycle частичного и полного refund в admin-панели. 2026-08-10.
  - Что сделать: recheck должен открыть возврат, partial refund сохранить новый refundable amount после reload, следующий refund использовать остаток, а fully refunded payment остаться terminal с точным blocker.
  - Что сделано: admin UI удаляет stale amount override после успешного refund и проверяет нулевой остаток до общего status blocker. Stateful mock сохраняет order/payment/refund state и журнал двух возвратов. Desktop/mobile flow подтверждает `590 -> 390 -> 0`, payload/authorization, confirm-dialog, reload persistence, console errors и overflow.
  - Доказательство: frontend `112/112`, typecheck/build, audit `0 vulnerabilities`; targeted payment refund lifecycle desktop/mobile `2/2`, полный console-responsive Playwright `98/98`, all-screens `6/6`; backend `1112/1112`, fresh local SQLite checkout/payment/subscription/VPN smoke и secret scan `657/0`. Реальные provider sandbox/live кабинеты и внешние VPS/staging/3x-ui/Telegram/SMTP evidence не закрывались.
...
| 2026-08-09 | Codex | Cabinet support request ordering, late logout response и private draft cleanup | Зеленое локально | Frontend `110/110`, cabinet desktop/mobile `14/14`, полный console-responsive suite `58/58`, typecheck/build и dependency audit; external live evidence не переиспользовалось |
...
| 2026-08-05 | Codex | QR SVG MIME/size validation и безопасный image render в cabinet/admin | Зеленое локально | Frontend `94/94`, changed desktop/mobile/admin `8/8`, полный responsive/console suite `16/16`, build/typecheck, backend `1112/1112`, EF drift guard, fresh SQLite smoke, audit/secret scan; external live evidence не переиспользовалось |
| 2026-08-05 | Codex | Payment return/provider/stored URL и public/cabinet safe-link boundary | Зеленое локально | Backend `1112/1112`, targeted SQLite `17/17`, frontend `92/92`, changed desktop/mobile `4/4`, полный responsive/console suite `16/16`, fresh SQLite smoke; external live evidence не переиспользовалось |
| 2026-08-05 | Codex | Shared clipboard success/failure, повторный запуск и responsive feedback | Зеленое локально | Frontend `91/91`, typecheck/build, cabinet desktop/mobile `2/2`, полный responsive/console suite `16/16`, fresh SQLite smoke, backend `1103/1103`; external live evidence не переиспользовалось |
| 2026-08-05 | Codex | Checkout single-flight и повторная оплата без popup | Зеленое локально | Frontend `90/90`, typecheck/build, changed Playwright `4/4`, полный responsive/console suite `16/16`, fresh SQLite smoke, backend `1103/1103`; external live evidence не переиспользовалось |
| 2026-08-05 | Codex | Server-owned checkout context, provider readiness, anonymous order privacy | Зеленое локально | Backend `1103/1103`, targeted `55/55`, frontend `90/90`, responsive Playwright `16/16`, fresh SQLite smoke; external live evidence не переиспользовалось |
| 2026-08-05 | Codex | Durable email delivery, защищенный reset-код, retry/admin visibility | Зеленое локально | Backend `1098/1098`, targeted `40/40`, frontend `90/90`, responsive Playwright `16/16`, fresh SQLite smoke; real SMTP/VPS/staging evidence остается открытым |
...
| --- | --- | --- | --- | --- | --- |
| `BUG-2026-08-10-011` | P1 | Admin payments/refund UI | После partial refund форма сохраняла старую сумму выше нового max; fully refunded payment показывал неточный status blocker, а mock GET не сохранял refund state. | Исправлено локально | Stateful partial/full refund lifecycle проходит desktop/mobile `2/2`; полный Playwright `98/98`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-010` | P1 | Admin subscriptions/E2E | Subscription actions возвращали DTO, но extend/sync/migrate state не сохранялся в mock GET; activate и terminal cancel/reload lifecycle оставались недоказанными. | Исправлено локально | Stateful subscription lifecycle проходит desktop/mobile `2/2`; полный Playwright `96/96`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-009` | P1 | Admin VPN access/E2E | Access actions возвращали DTO, но mock GET после reload снова выдавал исходный Active state; enable lifecycle, persisted status и revision sequence оставались недоказанными. | Исправлено локально | Stateful access lifecycle проходит desktop/mobile `2/2`; полный Playwright `94/94`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-008` | P1 | Admin 3x-ui clients/E2E | Browser gate проверял migration и открывал reset-dialog, но не выполнял enable/disable/sync/reset-traffic и не доказывал сохранение client state после reload. | Исправлено локально | Stateful client lifecycle проходит desktop/mobile `2/2`; полный Playwright `92/92`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-007` | P1 | Admin provisioning/E2E | Browser gate рендерил provisioning runs, но не выполнял health/precheck/deploy/cancel/retry/support actions и не доказывал безопасные validation mode/risk warnings после reload. | Исправлено локально | Stateful validation lifecycle проходит desktop/mobile `2/2`; полный Playwright `90/90`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-006` | P1 | Admin VPN infrastructure/E2E | Browser gate показывал серверы, панели и inbound-ы, но не выполнял их managed lifecycle и не доказывал write-only поведение SSH/panel credentials после reload. | Исправлено локально | Stateful infrastructure lifecycle проходит desktop/mobile `2/2`; полный Playwright `88/88`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-005` | P1 | Admin Telegram settings/E2E | Browser gate выполнял readiness check, но не сохранял настройки и не доказывал reload persistence, очистку secret inputs и отсутствие raw bot/webhook tokens в DTO/DOM. | Исправлено локально | Secure stateful save/check/reload/edit lifecycle проходит desktop/mobile `2/2`; полный Playwright `86/86`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-004` | P1 | Admin payment providers/E2E | Browser gate проверял readiness существующего аккаунта, но не выполнял create/edit/enable lifecycle и не доказывал отсутствие raw secrets в DTO/DOM после reload. | Исправлено локально | Secure stateful lifecycle проходит desktop/mobile `2/2`; полный Playwright `84/84`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-003` | P1 | Cabinet Telegram/support E2E | Browser gate показывал Telegram и support status controls, но не выполнял link/unlink и close/reopen, поэтому URL contract, authorization, optimistic revision и reload persistence оставались недоказанными. | Исправлено локально | Stateful desktop/mobile lifecycle проходит `2/2`; полный Playwright `82/82`, frontend/backend suite и fresh SQLite smoke зелёные. |
| `BUG-2026-08-10-002` | P1 | Admin managed configuration/E2E | Тарифы, реферальные программы, сценарии, релизы, FAQ и контент имели API unit-контракты и render smoke, но browser gate не доказывал полный create/edit/delete lifecycle, confirm-dialog и сохранение результата после reload. | Исправлено локально | Stateful UI CRUD проходит desktop/mobile; полный Playwright `80/80`, frontend/backend suite и fresh SQLite smoke зелёные. |
...
| `BUG-2026-08-09-003` | P0 | Cabinet support lifecycle | Медленный ответ старого обращения мог заменить сообщения нового выбранного thread или вернуться после logout; support/auth/reset drafts сохранялись в памяти для следующего входа. | Исправлено локально | Request/session generation, single-owner create reload, immediate thread cleanup и полный private-state reset покрыты desktop/mobile E2E. |
...
| `BUG-2026-08-05-064` | P0 | Frontend API response boundary | Успешный HTTP-ответ без JSON-контракта слепо приводился к `T`; HTML proxy/login page, пустой body, malformed JSON или JSON-примитив могли дойти до `.map/.filter` и обрушить рендер страницы. | Исправлено локально | Строгая проверка JSON MIME, непустого структурированного payload и controlled `502` покрыты unit и desktop/mobile Playwright без `pageerror`. |
| `BUG-2026-08-05-063` | P0 | Frontend API lifecycle | JSON и SVG transport ожидали fetch и чтение body без deadline; зависший backend/proxy мог навсегда оставить checkout, auth, cabinet или admin операцию в busy/loading состоянии без возможности повторить. | Исправлено локально | Общий AbortController deadline до полного body, controlled timeout error и cleanup timer/listener покрыты transport unit regressions и полным browser suite. |
| `BUG-2026-08-05-062` | P0 | Admin readiness navigation | Неизвестный server-driven `actionHref` давал `null` section, но условие `!actionSection` считало его допустимым и вставляло произвольный адрес, включая исполняемую схему, в активный `<a>`. | Исправлено локально | Exact fragment allow-list, capability check и канонический href покрыты unit и desktop/mobile Playwright; исполняемая ссылка отсутствует в DOM. |
| `BUG-2026-08-05-061` | P0 | QR SVG render boundary | Пять cabinet/admin QR-preview вставляли API SVG через `dangerouslySetInnerHTML`, а text client не проверял MIME, пустой ответ и размер; измененный или скомпрометированный ответ мог стать активным DOM. | Исправлено локально | Строгий response boundary, пассивный SVG validator, изолированный `<img>` render и controlled rejection покрыты unit и desktop/mobile Playwright. |
| `BUG-2026-08-05-060` | P0 | Payment external URL boundary | `returnUrl` принимался без проверки, provider redirect сохранялся как confirmation URL, а public/cabinet напрямую создавали ссылку и copy-команду из API/DB значения, включая исполняемые и credential-bearing схемы. | Исправлено локально | Backend fail-closed URL validation, отсутствие unsafe persistence/exposure и shared frontend warning покрыты SQLite unit и desktop/mobile Playwright. |
| `BUG-2026-08-05-059` | P1 | Shared clipboard UI | 12 copy-кнопок объявляли успех при отсутствующем Clipboard API; permission rejection оставался необработанным, а быстрый повтор мог запустить несколько записей. | Исправлено локально | Проверенный clipboard result, in-flight guard, visible stable feedback и unmount cleanup покрыты unit и desktop/mobile Playwright. |
| `BUG-2026-08-05-058` | P1 | Checkout/payment UX | Пока создавалась checkout-сессия, соседний тариф оставался активным и мог запустить второй запрос; retry payment вызывал `window.open` после `await`, поэтому popup мог быть заблокирован при ложном сообщении об успехе. | Исправлено локально | Общий checkout busy-state, synchronous in-flight guard и явная карточка retry payment покрыты unit и desktop/mobile Playwright; live payment redirect остается внешней проверкой. |
| `BUG-2026-08-05-057` | P0 | Checkout trust boundary | Public/cabinet принимали клиентский channel и `IsFirstPurchase`, public checkout сохранял заведомо неоплачиваемые сессии, а анонимный status раскрывал `UserId`, тариф и сумму по GUID заказа. | Исправлено локально | Серверный Web context, вычисление первой покупки, provider readiness до записи, `410 Gone` для anonymous status и API/browser/SQLite regressions. |
| `BUG-2026-08-05-056` | P0 | Email delivery lifecycle | Outbox создавал pending `NotificationDelivery`, но production reader отсутствовал; reset payload не содержал рабочего кода, поэтому письмо нельзя было отправить. | Исправлено локально | Durable claim/lease/backoff worker, SMTP boundary, protected reset token, masked admin monitoring/retry и SQLite/browser regressions; факт real SMTP delivery остается внешней проверкой. |
| `BUG-2026-08-05-055` | P0 | Referral lifecycle | Реферальный код не принимался при регистрации, production-код не создавал relationship/ledger, admin JSON не исполнялся, а кабинет возвращал source user, program id и metadata. | Исправлено | Atomic attribution, durable idempotent reward outbox, strict program validation, redacted cabinet DTO и структурированный admin UI покрыты SQLite/API/Playwright. |
| `BUG-2026-08-05-054` | P0 | Promo lifecycle | Неизвестный или ограниченный промокод молча игнорировался; период, allow-list и redemption limits не проверялись, `PromoCodeId`/free days не сохранялись, а последний limit slot можно было занять конкурентно. | Исправлено | Fail-closed checkout/order validation, serialized relational redemption, paid promo snapshot, subscription/renewal duration и HTTP/UI regressions покрыты backend, SQLite и Playwright. |
| `BUG-2026-08-05-053` | P0 | Checkout claim concurrency | Заказ сохранялся до привязки checkout-сессии; concurrent users могли оба получить success и создать два заказа, после чего последний save перезаписывал owner. Повторный claim completed-сессии также менял её на expired. | Исправлено | Conditional reservation, order creation и final session link выполняются в одной transaction; SQLite race проверяет один winner order, idempotent same-user response и отказ другому user без partial rows. |
| `BUG-2026-08-05-052` | P0 | Provisioning cancel concurrency | Cancel выполнял read-then-tracked-save без version predicate и мог перезаписать concurrent worker claim, вернуть node в `New` и скрыть уже начавшееся внешнее выполнение. | Исправлено | Transactional conditional `status + UpdatedAt` cancel возвращает retryable conflict при проигранной гонке; SQLite fault injection проверяет отсутствие partial node/audit mutations. |
| `BUG-2026-08-05-051` | P0 | Provisioning retry concurrency | `RetryAsync` публиковал claimable queue status первым коммитом и отдельным коммитом менял его на `Retrying`; worker мог обработать run между ними, после чего API перезаписывал processing/terminal status. | Исправлено | Retry создается как `Retrying` вместе с node/step/log/audit за один commit; SQLite interceptor и worker/coordinator regressions покрывают boundary. |
