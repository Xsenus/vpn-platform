...
- [x] `STATE-001` Backend test suite проходит: `592/592`.
...
- [x] `STATE-014` Roadmap и текущие статусные документы синхронизированы с проверками 2026-06-14.
  - Что сделано: верхний статус roadmap, README, final runbook, release decision, changelog, TEST_RESULTS, product/admin UI roadmap и seed "Что нового" приведены к одному состоянию: backend `592/592`, frontend `66/66`, browser console smoke `9/9`, latest release `2026-06-23-admin-bootstrap-readiness-password-env-validator`, версия `0.288.0`.
  ...
  - Доказательство: `RoadmapCurrentStateTests` 2/2, `BugRegisterConsistencyTests` 2/2, `ProvisioningSecretStatusConsistencyTests` 1/1, `ProductAdminUiRoadmapSyncTests` 1/1, `ProductionReadinessGateTests` 57/57, `VpsProductionSmokeTests` 7/7, `StagingSmokeChecklistTests` 8/8, `PaymentProviderSmokeReportTests` 6/6, `AdminVpsSmokeReportTests` 15/15, `AdminBootstrapCliScriptTests` 11/11, `VpnLiveSmokeReportTests` 4/4, `ChannelWebhooksControllerTests` 2/2, `ReadmeDocumentationTests`, `FinalDocsChangelogTests`, `DocumentationEncodingTests`, local SQLite VPS smoke dry-run, fresh local SQLite smoke, local admin browser smoke через end-to-end wrapper, local CLI bootstrap admin smoke, admin bootstrap readiness password env validator regression, admin bootstrap password env name guard regressions, direct admin bootstrap non-local reset guard regression, admin bootstrap provider normalization regression, admin bootstrap profile normalization regression, admin VPS workspace path normalization regression, admin VPS report path normalization regression, admin VPS bootstrap password env normalization regression, local bootstrap smoke cleanup reliability regression, admin VPS smoke wrapper identity normalization regression, admin VPS bootstrap wrapper admin email normalization regression, admin VPS bootstrap wrapper URL normalization regression, admin VPS bootstrap readiness URL normalization regression, admin VPS bootstrap readiness admin email normalization regression, admin VPS bootstrap readiness environment default regression, admin VPS bootstrap readiness provider normalization regression, admin VPS bootstrap readiness provider mode regression, admin VPS bootstrap readiness ready flag regression, admin VPS bootstrap provider guard regression, admin VPS release id known guard regressions, admin VPS environment default regressions, admin VPS operator default regressions, admin VPS report path guard regressions, admin VPS email guard regressions, admin VPS URL guard regressions, local bootstrap port guard regression, max duration format guard regressions, standalone evidence explicit max duration guard regressions, local/admin/production env upper-bound max duration regressions, local bootstrap explicit max duration guard regression, local bootstrap max duration propagation regression, local bootstrap wrapper fail-fast regression, local bootstrap env max duration regression, admin VPS smoke explicit max duration guard regression, admin VPS bootstrap env max duration guard regression, admin VPS bootstrap env max duration regression, admin VPS smoke env max duration regression, latest release DateTimeOffset sorting regression, admin VPS smoke wrapper max duration propagation regression, admin VPS bootstrap smoke wrapper max duration propagation regression, admin VPS smoke evidence max duration guard regression, admin VPS bootstrap smoke evidence max duration guard regression, admin VPS smoke evidence chronology summary regression, admin VPS bootstrap evidence chronology summary regression, admin VPS bootstrap evidence report id timestamp link regression, admin VPS smoke evidence report id timestamp link regression, admin VPS smoke evidence report id timestamp regression, admin VPS bootstrap evidence report id timestamp regression, admin VPS bootstrap evidence report id prefix regression, admin VPS smoke evidence report id prefix regression, admin VPS smoke evidence report id uniqueness regression, admin VPS smoke evidence gate flags summary regression, admin VPS smoke evidence status counts summary regression, admin VPS smoke evidence report id summary regression, admin VPS smoke evidence identity summary regression, admin VPS smoke evidence duration order regression, admin VPS smoke evidence duration summary regression, admin VPS smoke evidence expected fingerprint regression, admin VPS bootstrap evidence expected fingerprint regression, admin VPS bootstrap evidence fingerprint summary regression, admin VPS bootstrap evidence duration summary regression, admin VPS bootstrap evidence smoke summary regression, admin VPS bootstrap evidence timing summary regression, admin VPS bootstrap evidence readiness inputs summary regression, admin VPS bootstrap evidence reset flags summary regression, admin VPS bootstrap evidence status summary regression, admin VPS bootstrap evidence operator summary regression, admin VPS bootstrap evidence identity summary regression, admin VPS bootstrap evidence sections summary regression, admin VPS smoke evidence sections summary regression, admin VPS bootstrap evidence preflight summary regression, admin VPS smoke evidence preflight summary regression, admin VPS bootstrap readiness preflight timing link regression, admin VPS bootstrap readiness smoke path link regression, admin VPS bootstrap smoke timing link regression, admin VPS smoke evidence timing link regression, admin VPS bootstrap readiness metadata link regression, admin VPS bootstrap readiness chain validation regression, admin VPS bootstrap readiness self-validate regression, admin VPS bootstrap smoke report self-validate regression, admin VPS bootstrap smoke environment link regression, admin VPS bootstrap smoke admin email link regression, admin VPS smoke report self-link regression, admin VPS smoke preflight self-link regression, admin VPS bootstrap smoke report self-link regression, admin VPS bootstrap readiness report link regression, admin VPS smoke admin email evidence regression, admin VPS smoke preflight release id regression, admin VPS smoke unified release id regression, admin VPS bootstrap smoke readiness path link regression, admin VPS bootstrap smoke report release link regression, admin VPS bootstrap smoke release id chain regression, admin VPS bootstrap smoke route regression, admin VPS smoke report route contract regression, admin VPS smoke sections contract regression, admin VPS bootstrap smoke evidence validator regression, admin VPS bootstrap smoke readiness regression, admin VPS bootstrap smoke report validator, admin VPS bootstrap smoke wrapper regression, admin VPS smoke evidence validator regression, backend full suite `592/592`, frontend tests `66/66`, latest "Что нового" `2026-06-23-admin-bootstrap-readiness-password-env-validator`.
