...
- [x] Roadmap progress синхронизирован с master roadmap: `600/620` closed, readiness `96.8%`, `20` remaining, `19` open, `1` in progress, `0` blocked.
...
- [x] 3x-ui client actions проходят disable/reload/enable/sync/reset-traffic lifecycle на desktop/mobile; cabinet/admin QR cache очищается до повторного GET и после local blocker, технические API-коды из `error`, `message` и plain text заменяются безопасным fallback.
...
- [x] Latest "Что нового": `2026-08-10-api-error-payload-hardening`, версия `0.587.0`; machine codes и пустые строки во всех поддерживаемых error payload не попадают в UI, неизвестный auth code использует fallback.
