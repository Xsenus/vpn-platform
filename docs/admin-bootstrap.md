...
Для одного операторского прохода можно сначала выполнить bootstrap/reset, а затем сразу прогнать admin VPS smoke под этим же аккаунтом. Пароль берется только из env, в выводе пишется `Password: [hidden]`, а для не-локальной БД нужен явный `-ConfirmBootstrapReset`. Если переопределяете `-AdminPasswordEnvName`, имя должно быть безопасным env identifier и содержать `PASSWORD`; unsafe значения отклоняются до чтения process env.
...
`scripts/test-admin-vps-bootstrap-smoke-evidence-latest-release-guard.ps1` proves that paired bootstrap readiness/smoke evidence with stale `releaseId` is rejected before the evidence chain can be accepted:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\test-admin-vps-bootstrap-smoke-evidence-latest-release-guard.ps1
```
...
Внутри этого flow `scripts/admin-vps-bootstrap-smoke.ps1` вычисляет latest release один раз, fail-fast отклоняет ручной `ReleaseId`, которого нет в `backend/src/VpnPlatform.Api/AppReleases/releases.json`, неподдерживаемый non-local `Provider` и unsafe `AdminPasswordEnvName` до readiness/bootstrap/smoke artifacts; `-LocalSqlite` всегда передает канонический provider `Sqlite`. Wrapper передает `ApiBaseUrl`, `AdminWebUrl`, `AdminEmail`, `AdminPasswordEnvName`, `SmokeReportPath`, `PreflightReportPath`, `ReadinessReportPath`, `BootstrapSmokeReportPath`, `ProjectPath` и `FrontendPath` в readiness gate, `scripts/admin-vps-smoke.ps1` и итоговый bootstrap smoke report без окружающих пробелов, а один `releaseId` - в readiness gate, `scripts/admin-vps-smoke.ps1` и итоговый bootstrap smoke report. Standalone readiness дополнительно fail-fast отклоняет неизвестный ручной `ReleaseId`, нормализует регистронезависимые `Postgres`/`Sqlite` значения перед записью sanitized report, поэтому `postgres` из CLI/env сохраняется как `Postgres`, а неподдерживаемые значения все еще падают на `provider-supported`; пустой или whitespace `EnvironmentName` сохраняется как `Production`, unsafe password env names падают на `password-env-name-safe` без чтения env, а `ApiBaseUrl`, `AdminWebUrl`, `AdminEmail`, `AdminPasswordEnvName`, report paths и workspace paths записываются/используются без окружающих пробелов. Browser smoke report пишет `smokeReportPath`, readiness report пишет `readinessReportPath`, итоговый bootstrap smoke report пишет `readinessReportPath` и `bootstrapSmokeReportPath`, а evidence validators сверяют эти self-link поля с фактическими readiness/smoke/bootstrap JSON. Standalone validators readiness и итогового bootstrap reports также сверяют `readinessReportPath`/`bootstrapSmokeReportPath` с фактическим `-ReportPath` до paired evidence validation; readiness validator дополнительно отклоняет tampered `passwordEnvName`, если он не является безопасным env identifier с `PASSWORD`. Bootstrap report validator дополнительно валидирует связанный readiness report, сверяет readiness `smokeReportPath`/`preflightReportPath`, `apiBaseUrl`, `adminWebUrl`, `environmentName`, `operator`, `adminEmail`, `releaseId`, `provider`, `passwordEnvName`, `localSqlite` и `confirmBootstrapReset` с readiness/preflight/browser smoke reports и требует порядок readiness `generatedAt` -> preflight `generatedAt` -> smoke `completedAt` -> bootstrap report `generatedAt`, чтобы нельзя было смешать bootstrap одного окружения, admin-аккаунта, режима БД или времени выполнения со smoke другого. Evidence validator отклоняет readiness/bootstrap пару с разным `releaseId`, а preflight report не проходит validation с пустым `releaseId`.

Standalone readiness release guard:

```powershell
powershell -ExecutionPolicy Bypass -File scripts\test-admin-vps-bootstrap-readiness-release-guard.ps1
```
