...
Standalone bootstrap evidence validator fail-fast отклоняет `MaxEvidenceChainMinutes <= 0` и `> 1440` едиными сообщениями до чтения readiness/bootstrap/preflight/smoke evidence; regression покрывает `bad-max-evidence-chain-minutes` и `too-high-max-evidence-chain-minutes`.
