...
Внутри этого flow `scripts/admin-vps-bootstrap-smoke.ps1` вычисляет latest release один раз и передает один `releaseId` в readiness gate, `scripts/admin-vps-smoke.ps1` и итоговый bootstrap smoke report. Browser smoke report пишет `smokeReportPath`, итоговый bootstrap smoke report пишет `readinessReportPath` и `bootstrapSmokeReportPath`, а evidence validators сверяют эти self-link поля с фактическими readiness/smoke/bootstrap JSON. Standalone validator итогового bootstrap report также сверяет `bootstrapSmokeReportPath` с фактическим `-ReportPath` до paired evidence validation. Итоговый bootstrap smoke report связывает свои `apiBaseUrl`, `adminWebUrl`, `environmentName`, `operator` и `adminEmail` с preflight и browser smoke reports, чтобы нельзя было смешать bootstrap одного окружения или admin-аккаунта со smoke другого. Evidence validator отклоняет readiness/bootstrap пару с разным `releaseId`, а preflight report не проходит validation с пустым `releaseId`.
