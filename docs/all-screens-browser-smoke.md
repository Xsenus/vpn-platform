...
- `npm run e2e:console --prefix frontend`: `148/148`; all-screens: `6/6`.
...
- Latest "Что нового": `2026-08-11-cabinet-auth-data-boundary`, версия `0.601.0`; до авторизации cabinet не показывает ложные aggregate/private empty states, после login фактические данные появляются без reload, а полный browser gate проходит `148/148` без failed/flaky/skipped.
