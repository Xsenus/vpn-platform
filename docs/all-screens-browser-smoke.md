...
- Полный Playwright public/cabinet/admin/all-screens/mobile/console baseline остаётся `282/282`: стандартный run подтвердил `281/282`, а единственный default-timeout multi-route WCAG smoke после scoped 60-second boundary прошёл в полном изменённом all-screens subset `14/14` за `16.4 min`. Frontend `197/197`, production build и reviewed admin bundle budget raw `587524/587776`, gzip `156320/156672` зелёные.
...
- Latest "Что нового": `2026-08-31-production-vpn-panel-smoke`, версия `0.756.0`; frontend baseline остаётся `197/197`, полный изменённый all-screens responsive-прогон проходит `14/14` за `16.4 min`, production admin smoke прошел `17/17` sections, real 3x-ui panel/inbound/node checks `3/3`.
