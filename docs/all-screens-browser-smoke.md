...
- `npm run e2e:console --prefix frontend`: `162/162`; all-screens: `6/6`.
...
- Latest "Что нового": `2026-08-11-public-landing-load-lifecycle`, версия `0.608.0`; главная выполняет по одной initial FAQ/CMS попытке под StrictMode, показывает отдельное FAQ recovery-состояние и отклоняет stale completion, а полный browser gate проходит `162/162` без failed/flaky/skipped.
