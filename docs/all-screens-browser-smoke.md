...
- Полный Playwright public/cabinet/admin/all-screens/mobile/console `282/282` за `13.0 min`, frontend `196/196`, production build и admin bundle budget raw `586744/586752`, gzip `156174/156672` зелёные.
...
- Latest "Что нового": `2026-08-15-server-archive-provisioning-readonly`, версия `0.748.0`; archived VPN server и его historical provisioning runs сохраняют diagnostics без mutation controls, а worker/direct commands не меняют terminal state; desktop/mobile и полная responsive-матрица проходят без invalid geometry.
